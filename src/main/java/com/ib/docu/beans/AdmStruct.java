...
@Named
@ViewScoped
public class AdmStruct extends IndexUIbean   {
	...
	public void actionSaveZveno() {
		...
		try {
			...
			referent.setRefType(DocuConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO);
	...
	public void actionSaveSluj() {
		...
		try {
			...
			referentSluj.setRefType(DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL);
...
public void actionSelectNew(ActionEvent event) {
		...
		if(event!=null && selectedNode!=null) {
			...
			referent.setRefRegistratura(getUserData(UserData.class).getRegistratura());
			//Integer codeForRegistr=0; // за да сетнем по подразбиране регистратурата на родителя
...
//			if(codeForRegistr!=null && !codeForRegistr.equals(0)) { 
//				
//				try {
...
//						if(selectedZvenoClassif!=null) {
//							referent.setRefRegistratura( (Integer) selectedZvenoClassif.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA]);
//						}
//					} catch (DbErrorException e) {
//					LOGGER.error(e.getMessage(), e);
//					JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_ERROR, e.getMessage());	
//				}
//			}else	{ //ако звеното няма родител, слагаме текущата регисттратура
//				referent.setRefRegistratura(getUserData(UserData.class).getRegistratura());
//			}	
		}
	}
	...
	public void selectForEdit(Object[] item) {
		viewTab = true;
		if(SearchUtils.asInteger(item[2]).equals(DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL)) {
...
//				Integer zvenoType = (Integer) selectedZvenoClassif.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE];
//				if(zvenoType != null && zvenoType.intValue() == DocuConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO) {
					...
					if(selectedZvenoClassif!=null && selectedZvenoClassif.getCodePrev()!=0) {
				    	...
				    	registrParent = (Integer) selectedZvenoClassif.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA];
...
//				}
	...
	public void searchHistoryZveno() {
		searchHistory(referent.getCode(), DocuConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO);
		isZveno = true;
	}
	
	public void searchHistoryEmpl() {
		searchHistory(referentSluj.getCode(), DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL);
		isZveno = false;
	}
	...
	private boolean checkForLock() {
		...
		try { 
			Object[] obj = daoL.check(ud.getUserId(), DocuConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT, DocuConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT);
			if (obj != null) {
				 res = false;
	...
	public void lockAdmStr() {	
		...
		try { 
			JPA.getUtil().runInTransaction(() -> {
				daoL.lock(ud.getUserId(), DocuConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT, DocuConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT, null);
			});
		} catch (DbErrorException e) {
	...
	public void actionLoadLeft() {
		try {
			leftEmpl = getSystemData().getChildrenOnNextLevel(DocuConstants.CODE_CLASSIF_ADMIN_STR_REPORTS, -1000, decodeDate, getCurrentLang());
		} catch (DbErrorException e) {
	...
	public void actionSaveLeftEmpl() {
			...
			try {
				...
				SystemClassif classifEmpl = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_ADMIN_STR_REPORTS, selectedEmpl, getCurrentLang(), decodeDate, false);
