		...
		if(idDocs != null && !idDocs.isEmpty()) {
			...
			try {
				...
				for (Long docId : idDocs) {
					...
					filesListDoc = new FilesDAO(getUserData()).selectByFileObjectDop(SearchUtils.asInteger(doc[0]), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
					
					if (!filesListDoc.isEmpty()) {
						for (Files file : filesListDoc) {
							if (file.getFilePurpose() != null && file.getFilePurpose().equals(DocuConstants.CODE_ZNACHENIE_FILE_PURPOSE_MAIN_DOC)) {
				...
				if (selMenu.intValue() == DocuConstants.CODE_ZNACHENIE_DASHBOARD_FOR_SAGL) {
					this.opinionLst = getSystemData().getClassifByListVod(DocuConstants.CODE_LIST_TASK_TYPE_TASK_OPINION, Integer.valueOf(DocuConstants.CODE_ZNACHENIE_TASK_TYPE_SAGL), getCurrentLang(), new Date());
					this.endOpinion = DocuConstants.CODE_ZNACHENIE_TASK_OPINION_SAGL;					
				}
				
				if (selMenu.intValue() == DocuConstants.CODE_ZNACHENIE_DASHBOARD_FOR_PODPIS) {
					this.opinionLst = getSystemData().getClassifByListVod(DocuConstants.CODE_LIST_TASK_TYPE_TASK_OPINION, Integer.valueOf(DocuConstants.CODE_ZNACHENIE_TASK_TYPE_PODPIS), getCurrentLang(), new Date());
					this.endOpinion = DocuConstants.CODE_ZNACHENIE_TASK_OPINION_PODPIS;
				}
	...
	public void actionChangeOpinion() {
		...
		try {
			...
			if (this.endOpinion == null) {
				...
				if (getSystemData().matchClassifItems(DocuConstants.CODE_CLASSIF_TASK_OPINION_WITH_COMMENT, this.endOpinion, new Date())
