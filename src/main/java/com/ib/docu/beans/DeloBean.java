...
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_DELO_STATUS_COMPLETED;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_DELO_STATUS_CONTINUED;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_DELO_TYPE_NOM;
...
import com.ib.docu.db.dao.DeloDeloDAO;
import com.ib.docu.db.dao.DeloDocDAO;
import com.ib.docu.db.dao.DeloStorageDAO;
...
//			}
		...
		try {
			if(Integer.valueOf(getSystemData().getSettingsValue("delo.deloWithToms"))!=DocuConstants.CODE_ZNACHENIE_DA) {
				visibleTom = false;
			}
			if(delo.getId() != null) {
	...
	public void actionStatusDeloChange(boolean check) {
		if (check) {
			if (delo.getStatus()!=null && (delo.getStatus()==DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_DA || delo.getStatus()==DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_UA)) {
		...
		if (Objects.equals(this.delo.getStatus(),DocuConstants.CODE_ZNACHENIE_DELO_STATUS_COMPLETED)) {
			try {
				//настройка -  При приключване на преписка се приключват задачите към нейните документи и вложените преписки 
				Integer s1 = ((SystemData)getSystemData()).getRegistraturaSetting(this.delo.getRegistraturaId(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_10);
				if( Objects.equals(s1,  DocuConstants.CODE_ZNACHENIE_DA) &&
					getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_COMPLETE_REGISTRATURA_TASKS)) {
	...
	private void loadDelo(Integer deloId) {
		...
		if(deloId != null){			
			try {
				...
				originalStatus=delo.getStatus();
				if (this.delo.getDeloType()!=null && this.delo.getDeloType()==DocuConstants.CODE_ZNACHENIE_DELO_TYPE_NOM) {
					...
					try {
						...
						if (delo.getFreeAccess()!=null && delo.getFreeAccess().equals(DocuConstants.CODE_ZNACHENIE_DA)) {
	...
	private boolean checkForLock(Integer idObj) {
		...
		try { 
			Object[] obj = daoL.check(getUd().getUserId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO, idObj);
			if (obj != null) {
				 res = false;
	...
	public void lockDelo(Integer idObj) {	
		...
		try { 
			JPA.getUtil().runInTransaction(() -> 
				daoL.lock(getUd().getUserId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO, idObj, null)
			);
		} catch (BaseException e) {
	...
	public boolean actionDeloNew() {
		...
		try {
			...
			this.delo.setStatus(DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ACTIVE);
			...
			obshtodostap = true; 
			Integer s1 = ((SystemData)getSystemData()).getRegistraturaSetting(delo.getRegistraturaId(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_12);
			if( Objects.equals(s1,  DocuConstants.CODE_ZNACHENIE_NE)) {
				obshtodostap = false; 
				this.delo.setFreeAccess(DocuConstants.CODE_ZNACHENIE_DA);
			}
			
			Integer tipDelo = ((SystemData)getSystemData()).getRegistraturaSetting(this.delo.getRegistraturaId(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_14);
	...
	public void actionSave() {
		...
		if (obshtodostap) {
			this.delo.setFreeAccess(DocuConstants.CODE_ZNACHENIE_NE);
		}else {
			this.delo.setFreeAccess(DocuConstants.CODE_ZNACHENIE_DA);
		} 
		if (hasRazdeli) {
			this.delo.setWithSection(DocuConstants.CODE_ZNACHENIE_DA);
		}else {
			this.delo.setWithSection(DocuConstants.CODE_ZNACHENIE_NE);
		} 
		if (hasToms) {
			this.delo.setWithTom(DocuConstants.CODE_ZNACHENIE_DA);
		}else {
			this.delo.setWithTom(DocuConstants.CODE_ZNACHENIE_NE);
		} 
	...
	public boolean checkDataDelo() throws DbErrorException {
		...
		if (this.delo.getWithTom()!=null && this.delo.getWithTom()==DocuConstants.CODE_ZNACHENIE_DA) {
	...
	public void actionTypeChange() {
		if (this.delo.getId()!=null && originalDeloType!=null && this.delo.getDeloType()==DocuConstants.CODE_ZNACHENIE_DELO_TYPE_NOM && originalDeloType!=DocuConstants.CODE_ZNACHENIE_DELO_TYPE_NOM) {
			//nqma pravo saobshtenie
			JSFUtils.addMessage(DELOFORM+":statusDelo",FacesMessage.SEVERITY_ERROR,getMessageResourceString(beanMessages,"delo.statusType"));
	...
	public void actionRemoveCurrentDeloFromDelo() {
		...
		if (this.delo.getStatus()!=null && (this.delo.getStatus()==DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_UA || this.delo.getStatus()==DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_DA)) {
	...
	public void actionDostap() {
		...
		try {
			if (zvenaCodesList.isEmpty()) {
		        ...
		        specs.put(DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE, DocuConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO);
		        
		        List<SystemClassif> zvena = getSystemData().queryClassification(DocuConstants.CODE_CLASSIF_ADMIN_STR, null, decodeDate, getUserData().getCurrentLang(), specs);
...
//			}
			Integer s9 = ((SystemData)getSystemData()).getRegistraturaSetting(this.delo.getRegistraturaId(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_9);
			String dvijMsg = "";
		...
		try {
			...
			JPA.getUtil().runInTransaction(() -> {
				...
				if (docId.equals(delo.getInitDocId())) { // маха се иницииращия док
					...
					SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO, delo.getId(), ident.toString());
...
//			loadDeloDocsList();
	...
	public void actionRemoveDeloDelo(Object[] row) {
		if (SearchUtils.asInteger(row[4])!=null && (SearchUtils.asInteger(row[4])==DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_UA || SearchUtils.asInteger(row[4])==DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_DA)) {
		...
		if (selectedDeloP!=null && selectedDeloP[0]!=null) {
			...
			try {
				String msg = this.delo.getDeloDelo().getDelo().getRnDelo() +" / "+ DateUtils.printDate(this.delo.getDeloDelo().getDelo().getDeloDate()) +
						"; "+  getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_DELO_STATUS, this.delo.getDeloDelo().getDelo().getStatus(), getUserData().getCurrentLang(), new Date());
...
//			actionSave();
	...
	public String getRnFullDeloAudit() {
		...
		if(delo != null && delo.getId() != null) {
			...
			try {
				rnAudit = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_DELO_TYPE,delo.getDeloType(), getCurrentLang(), new Date())+": "+this.delo.getRnDelo() + "/" + DateUtils.printDate(this.delo.getDeloDate());
			} catch (DbErrorException e) {
				LOGGER.error(e.getMessage(), e);
				JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_ERROR, e.getMessage());
			}
