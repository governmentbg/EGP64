...
import com.ib.docu.db.dao.DeloDvijDAO;
...
//	private Date backupDateVrashtane;
	...
	private int newMetod = DocuConstants.CODE_ZNACHENIE_PREDAVANE_NA_RAKA;
		...
		if(bean != null) {
			if(!Objects.equals(bean.getRnFullDelo(), this.rnFullDelo)){ // значи за 1-ви път се отваря табчето
				...
				try {
					...
					this.withTomove = Objects.equals(param1, String.valueOf(DocuConstants.CODE_ZNACHENIE_DA));
				...
				try {
					...
					this.predavaneTypeList = // премахват се от списъка няколко опции - имейл, сеос, ссев, уеб еау и 'на друга регистратура'
						...
						.stream()
						.filter(item -> (Integer)item.getValue() != DocuConstants.CODE_ZNACHENIE_PREDAVANE_SEOS
										&& (Integer)item.getValue() != DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV
										&& (Integer)item.getValue() != DocuConstants.CODE_ZNACHENIE_PREDAVANE_WEB_EAU
										&& (Integer)item.getValue() != DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL
...
//		this.backupDateVrashtane = tempDvij.getReturnDate();
	...
	public void actionSaveNewClicked() {
		...
		if(this.useTomove() && this.delo.getWithTom() == DocuConstants.CODE_ZNACHENIE_DA) {
...
//				dvij.setReturnToDate(setTime(d.dataVrashtane, new Date())); // RETURN_TO_DATE
	...
	/**
	 * Направен е избор в полето 'Групи лица'.
	 * Обхожда лицате в групата и ги добавя като накрая изчиства полето.
	 */
	...
	/**
	 * Направен е избор в полето 'лице - свободен текст'.
	 * Накрая изчиства полето.
	 */
	...
	/**
	 * Извлича данни на лица по код на лице и го добавя към новите движения.
	 * @param codes кодовете от административната структура
	 */
	...
	public boolean selectedMetodSystem() {
		return this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_DRUGA_REGISTRATURA;
	}
	...
	/**
	 * При показване на иконки за вида получатели - лице нфл/фзл, потребител от адм. структура, произволен имейл
	 */
	public String getTypeIcon(int type) {
		switch(type) {
			...
			// неочакван случай - нито е от адм. стр., нито е лице, нито произволен имейл
			default : return "fas fa-user";
		}
	}
	
	/**
	 * Текстът при посочване на иконката за вида получатели - лице нфл/фзл, потребител от адм. структура, произволен имейл
	 */
	public String getTypeString(int type) {
		...
		if(classif != null) { // това е валидно само за лицате нфл/фзл
		...
		else { // неочакван случай - нито е от адм. стр., нито е лице, нито произволен имейл
			return "";
		}
	}
	...
	public String decodeCodeRefName(Integer codeRef) {
		...
		try {
			...
			else { // кодът не е намерен нито в адм. структура, нито в лицате, сигурно е счупен
				return null;
			}
		} catch (DbErrorException e) {
			LOGGER.error(getMessageResourceString(beanMessages, ERROR_KLASSIF), e);
	...
	/**
	 * Прверява дали подаден код на референт е на лице, а не от адм. структура.
	 * @param codeRef
	 */
...
//	
	...
	public class Dvijenie {
		...
		private String name;
		private Integer tomNomer;
		private String dopInfo; // за лицата от адм. структ е длъжността, за лицате е информацията им
		...
		public boolean equals(Object obj) {
	        ...
	        if (this.codeReferent == null && d.codeReferent == null) { // ако кодовете са null, значи са вкарани ръчно имейли/корепонденти
        		if(this.name != null && d.name != null) { // значи са ръчно вкарани лица
	        ...
	        else // кодовете не са null, значи не са вкарани ръчно като имейли/лица и трябва да ги има в системата
	...
	/**
	 * Проверка дали бутона за разглеждане нa данни за лице да се покаже -  ако е физ. лице, в екрана за разгл. са скрити всички лични данни   
	 * @param idRefForModal
	 * @return
	 */
