...
import com.ib.docu.search.DeloDvijSearch;
...
public class DeloPrepDvijList extends IndexUIbean implements Serializable {
	...
	void initData() {
		...
		try {
			...
			docTypeList = createItemsList(false, DocuConstants.CODE_CLASSIF_DELO_TYPE, this.currentDate, true);
			docStatusList = createItemsList(false, DocuConstants.CODE_CLASSIF_DELO_STATUS, this.currentDate, true);
			//зарежда правата спрямо регистратурата
			disableRegList=getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_FULL_VIEW);
			List<SelectItem> regList = createItemsList(false, DocuConstants.CODE_CLASSIF_REGISTRATURI_OBJACCESS, this.currentDate, true);
			
			//зарежда вида на предаване
			this.predavaneTypeList = // премахват се от списъка двете опции - имейл и сеос
			createItemsList(false, DocuConstants.CODE_CLASSIF_DVIJ_METHOD, new Date(), true)
			.stream()
			...
			this.withTome=Objects.equals(getSystemData().getSettingsValue("delo.deloWithToms"),String.valueOf(DocuConstants.CODE_ZNACHENIE_DA));
...
//		searchDeloPrep.getDocTypeArr()[0] = DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN; 
//		searchDeloPrep.getDocTypeArr()[1] = DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN;	
//		periodR = null;
//		periodRes = null;
//		periodAnswer = null;
//		periodAssign = null;
...
//		txtCorresp = "";
	...
	public  void actionAddSelectRefV(){
		...
		if(codeRefCorrSrchV != null) {
			...
			try {
				...
				tekst = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REFERENTS, codeRefCorrSrchV, getUserData().getCurrentLang(), new Date());
				tmpSc.setTekst(tekst);
				scListV.add(tmpSc);
			} catch (DbErrorException e) {
				LOGGER.error(e.getMessage(),e);	
	...
	public void actionChangeReg() {
		...
		if(searchDeloPrep.getRegistraturaId()!=null) {		
			specificsRegister = Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA,Optional.of(searchDeloPrep.getRegistraturaId()));
		}else {
