...
import com.ib.docu.db.dao.DocProtocolDAO;
...
public class DocArchive extends IndexUIbean implements Serializable {			
	...
	public void initData() {
		...
		try {
			...
			this.docTypeArr = new Integer[]{ DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN};		
			this.protocolDAO = new DocProtocolDAO(2, getUserData());	
			
			this.specificsRegister = new HashMap<>();
			this.specificsRegister.put(DocuClassifAdapter.REGISTRI_INDEX_DOC_TYPE, DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK);
			this.specificsRegister.put(DocuClassifAdapter.REGISTRI_INDEX_VALID, SysConstants.CODE_ZNACHENIE_DA);
			
			this.classifRegsList = getSystemData().queryClassification(getUserData(), DocuConstants.CODE_CLASSIF_REGISTRI, null, decodeDate,getCurrentLang(), this.specificsRegister);
			...
			if(protUA != null) {
				this.vidAndStatusArchive.put(Integer.valueOf(protUA), Integer.valueOf(DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_UA));
			}
			
			if(protDA != null) {
				this.vidAndStatusArchive.put(Integer.valueOf(protDA), Integer.valueOf(DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_DA));
			}		
				...
				// отключване на всички обекти за потребителя(userId) и заключване на док., за да не може да се актуализира от друг	
					...
					if (idObj != null) {
						...
						JPA.getUtil().runWithClose(() -> {
							...
							this.filesListForWork = new FilesDAO(getUserData()).selectByFileObject(this.docProtArchive.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
							
							this.filesListForOffic = new FilesDAO(getUserData()).selectByFileObject(this.docProtArchive.getWorkOffId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
	...
	public void actionNew() {
		...
		this.docProtArchive.setDocType(DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK);
		this.docProtArchive.setFreeAccess(DocuConstants.CODE_ZNACHENIE_NE);
		this.docProtArchive.setDocVid(this.docVid);
		
		author.setRoleRef(DocuConstants.CODE_ZNACHENIE_DOC_REF_ROLE_AUTHOR);
	...
	public void actionChangeAvtomNo() {
	   ...
	   public void actionChangeRegister() {
			...
			try {
				 if(this.docProtArchive.getRegisterId() != null) {
					 alg  = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_REGISTRI, this.docProtArchive.getRegisterId(), getUserData().getCurrentLang(), new Date(), DocuClassifAdapter.REGISTRI_INDEX_ALG);
				 }
	...
	public void actionDelReg() {
		...
		try {
			...
			officProt.setDocType(DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN);
			officProt.setDocVid(this.docVid);
	
			Integer docTypeReg  = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_REGISTRI, this.idRegister, getUserData().getCurrentLang(), new Date(), DocuClassifAdapter.REGISTRI_INDEX_DOC_TYPE);
			...
			JPA.getUtil().runInTransaction(() -> { // запис на док и файловете в обща транзакция
				...
				if (this.filesListForWork != null && !this.filesListForWork.isEmpty()) {
					...
					for (Files f : this.filesListForWork) {
						new FilesDAO(getUserData()).saveFileObject(f, officProt.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
					}
				}
	...
	public Object[] actionSearchOfficProtByRnDoc(String rnDoc) {
		...
		docType[0] = DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN;
...
//		}
	...
	private boolean checkForLock(Integer idObj) {
		...
		try { 
			Object[] obj = daoL.check(getUserData().getUserId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC, idObj);
	...
	public void lockDoc(Integer idObj) {	
		...
		try { 
			...
			JPA.getUtil().runInTransaction(() -> daoL.lock(getUserData().getUserId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC, idObj, null));
