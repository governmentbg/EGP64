...
import com.ib.docu.db.dao.DocDvijDAO;
...
//	private boolean viewPlik=false;
	...
	public void initTab() {
		...
		if(bean != null) {
			this.specRegistratura = Collections.singletonMap(DocuClassifAdapter.REG_GROUP_INDEX_REGISTRATURA, ((UserData) getUserData()).getRegistratura());
			this.specGroup  = new HashMap<>();
			specGroup.putAll(specRegistratura);
			specGroup.put(DocuClassifAdapter.REG_GROUP_INDEX_TYPE, DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP);
			
			this.specGroupSeos = Collections.singletonMap(DocuClassifAdapter.REG_GROUP_INDEX_TYPE, DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS);
			...
			if(!Objects.equals(bean.getRnFullDoc(), this.rnFullDoc)) { // значи за 1-ви път се отваря табчето 
				...
				try {
					...
					this.withExemplars = Objects.equals(param1, String.valueOf(DocuConstants.CODE_ZNACHENIE_DA));
				...
				try {
					...
					if(this.isView == 1) {
						this.newMetod = DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL;
						
						this.predavaneTypeList = // допуща се само имейл
							createItemsList(true, DocuConstants.CODE_CLASSIF_DVIJ_METHOD, new Date(), true)
							.stream()
							.filter(item -> (Integer)item.getValue() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL)
					...
					// актуализация
						...
						this.predavaneTypeList =
							createItemsList(true, DocuConstants.CODE_CLASSIF_DVIJ_METHOD, new Date(), true)
							.stream()
							.filter(item -> (Integer)item.getValue() != DocuConstants.CODE_ZNACHENIE_PREDAVANE_WEB_EAU)
							.collect(Collectors.toList());
						
						// СЕОС да се вижда само за Входящи и Собствени документи
						if((this.document.getDocType() != DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN && this.document.getDocType() != DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN)
								|| Integer.valueOf(this.systemData.getSettingsValue("system.useSEOS")) != DocuConstants.CODE_ZNACHENIE_DA) {
							this.predavaneTypeList.removeIf(item -> (Integer)item.getValue() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SEOS);
						}
						
						// ССЕВ да се вижда само за Входящи и Собствени документи
						if((this.document.getDocType() != DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN && this.document.getDocType() != DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN)
								|| Integer.valueOf(this.systemData.getSettingsValue("system.useSSEV")) != DocuConstants.CODE_ZNACHENIE_DA) {
							this.predavaneTypeList.removeIf(item -> (Integer)item.getValue() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV);
						}
					}
					
					// проверява се изрично дали има права да праща по имейл
					if(!getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_SEND_MAIL)) {
						this.predavaneTypeList.removeIf(item -> (Integer)item.getValue() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL);
					}
	...
	public void actionSaveEdit() {
		...
		if(dvij.getReturnDate() == null) {
			...
			dvij.setDvijEmail(this.tempDvij.getDvijMethod() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL ? this.tempDvij.getDvijEmail() : null);
			...
			if (this.document != null && this.document.getCompetence() != null && this.document.getCompetence().equals(DocuConstants.CODE_ZNACHENIE_COMPETENCE_FOR_SEND)){
				this.document.setCompetence(DocuConstants.CODE_ZNACHENIE_COMPETENCE_SENT);
				try {
					JPA.getUtil().runInTransaction(() -> updateCompetenceDoc(DocuConstants.CODE_ZNACHENIE_COMPETENCE_SENT, dvij));
				} catch (BaseException e) {
	...
	public void actionSaveNewClicked() {
		if(this.countOfficalFiles == 0) {
			if(this.newDvijenia.stream().anyMatch(
					d -> d.metodPredavane == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SEOS
						|| d.metodPredavane == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV
						|| d.metodPredavane == DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL)) {
			...
			else if(this.newDvijenia.stream().anyMatch(d -> d.metodPredavane == DocuConstants.CODE_ZNACHENIE_PREDAVANE_DRUGA_REGISTRATURA)) {
	...
	public void actionSaveNewDvij() {
		...
		try {
			...
			for(Dvijenie d : this.newDvijenia) {
				...
				if(this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_DRUGA_REGISTRATURA) {
					...
					Integer drugaRegistratura = (Integer) empl.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA];
					
					boolean alreadySent = this.dvijenieDao.alreadySentToForeignReg(this.document.getId(), drugaRegistratura);
					if(alreadySent) {
						String drugaRegistraturaIme = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_REGISTRATURI, drugaRegistratura, SysConstants.CODE_DEFAULT_LANG, new Date(), true).getTekst();
				...
				dvij.setDvijEmail((d.getMetodPredavane() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL 
									|| d.getMetodPredavane() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_MAIL_CUSTOM) ? d.getEmail() : null); // DVIJ_EMAIL
			...
			if (this.document != null && (this.document.getProcessed() == null || this.document.getProcessed().equals(DocuConstants.CODE_ZNACHENIE_NE))){
				...
				JPA.getUtil().runInTransaction(() -> updateProcessedDoc(DocuConstants.CODE_ZNACHENIE_DA, dvij));
			}
	...
	private void updateProcessedDoc(int processed, DocDvij dvij) throws DbErrorException {
		...
		if (dvij != null) {
			...
			SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC, this.document.getId(), ident.toString());
	...
	private void setInitValues() {
		...
		if(this.isView == 1) {
			...
			this.newMetod = DocuConstants.CODE_ZNACHENIE_PREDAVANE_NA_RAKA;
		}
	...
	public String getMethodIcon(int methodCode) {
		switch(methodCode) {
			case DocuConstants.CODE_ZNACHENIE_PREDAVANE_NA_RAKA : return "fas fa-hand-paper";
			case DocuConstants.CODE_ZNACHENIE_PREDAVANE_POSHTA : return "fas fa-envelope";
			case DocuConstants.CODE_ZNACHENIE_PREDAVANE_FAX : return "fas fa-fax";
			case DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL : return "fas fa-at";
			case DocuConstants.CODE_ZNACHENIE_PREDAVANE_KURIER : return "fas fa-mail-bulk";
			case DocuConstants.CODE_ZNACHENIE_PREDAVANE_DRUGA_REGISTRATURA : return "fas fa-file-export";
			case DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV : return "fas fa-shield-alt";
			case DocuConstants.CODE_ZNACHENIE_PREDAVANE_MAIL_CUSTOM : return "fas fa-envelope-square";
	...
	public String getTypeIcon(int type) {
		switch(type) {
			case DocuConstants.CODE_ZNACHENIE_REF_TYPE_NFL : return "fas fa-building";
			case DocuConstants.CODE_ZNACHENIE_REF_TYPE_FZL : return "fas fa-people-carry";
	...
	public String getTypeString(int type) {
		SystemClassif classif = null;
		try {
			classif = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_REF_TYPE, type, SysConstants.CODE_DEFAULT_LANG, new Date(), false);			
		} catch (DbErrorException e) {
			LOGGER.error(getMessageResourceString(beanMessages, ERROR_KLASSIF), e);
			JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_ERROR, e.getMessage());
		}
	...
	private String decodeCodeRefNameSeos(Integer codeRef) {
		...
		try {
			Date date = new Date();
			SystemClassif adm = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_EGOV_ORGANISATIONS, codeRef, SysConstants.CODE_DEFAULT_LANG, date, true);
			return adm.getTekst();
		} catch (DbErrorException e) {
			LOGGER.error(getMessageResourceString(beanMessages, ERROR_KLASSIF), e);
			JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_ERROR, e.getMessage());
	...
	public boolean showDeleteButton(DocDvij dvij) {
		...
		if(dvij.getDvijMethod() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL && dvij.getStatus() == DocuConstants.DS_SENT) {
			return false;
		}
		...
		if((dvij.getDvijMethod() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SEOS || dvij.getDvijMethod() == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV)
				&& dvij.getDvijMethod() == DocuConstants.DS_REGISTERED) {
			return false;
		}
	...
	public void ssevGetAllDocCoresps() throws DbErrorException {
		String guidSSEV = SearchUtils.asString(
				getSystemData().getItemSpecific(
					DocuConstants.CODE_CLASSIF_REGISTRATURI,
					this.document.getRegistraturaId(),
					SysConstants.CODE_DEFAULT_LANG,
					new Date(),
					DocuClassifAdapter.REGISTRATURI_INDEX_GUID_SSEV));
	...
	public void actionSelectGroupSluj() {
		...
		try {
			...
			String emplCodes = (String) getSystemData().getItemSpecific(
					DocuConstants.CODE_CLASSIF_GROUP_EMPL, 
					this.selectedGroupSluj, 
					SysConstants.CODE_DEFAULT_LANG, 
					new Date(), 
					DocuClassifAdapter.REG_GROUP_INDEX_MEMBERS);
	...
	private void addNewDvijeniaByCodeAdm(List<Integer> codes) {
		 ...
		 * com.ib.docu.system.TestSystemData.testClassifGroupEmployees()
		*/
		...
		try {
			for (int code : codes) {
					...
					if(drugaRegistratura.equals(getUserData(UserData.class).getRegistratura())) {
						JSFUtils.addGlobalMessage(
								...
								String.format(
										...
										((Integer) empl.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE]) == 1 ? "Звеното" : "Лицето", 
										empl.getTekst()));
						scrollToMessages();
						continue;
					}
				...
				predavane.setEmail((String) empl.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_CONTACT_EMAIL]);
				...
				predavane.setType((Integer) empl.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE] == 1 ? TYPE_ADM_STR_ZVENO : TYPE_ADM_STR_LICE);
				predavane.setMetodPredavane(this.newMetod);
				predavane.setDataPredavane(this.dataPredavane);
				predavane.setDataVrashtane((this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SEOS || this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV) ? null : this.dataVrashtane);
				predavane.setExemplar(this.newPredavaneEkz);
	...
	public void actionSelectKoresp() {
		if(validateDates(true, this.newMetod != DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV)) {
	...
	public void actionSelectKorespEdited() {
		if(!validateDates(true, this.newMetod != DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV)) return;
		...
		if(validateDates(true, true)) {
			try {
				...
				String korespCodes = (String) getSystemData().getItemSpecific(
						DocuConstants.CODE_CLASSIF_GROUP_CORRESP, 
	...
	private void addNewDvijeniaByCodeKoresp(List<Integer> codes) {
		/*
		 * повече информация можете да намерите на:
		 * com.ib.docu.system.TestSystemData.testClassifReferents()
		*/
		...
		try {
			for (int code : codes) {
				...
				String type = null;
				String eikEgn = SearchUtils.asString(koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_EIK_EGN]);
				if(this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV) {

					if((int) koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_REF_TYPE] == DocuConstants.CODE_ZNACHENIE_REF_TYPE_NFL) {
						identifikatorString = "ЕИК";
						type = DocuConstants.SSEV_TYPE_LEGALPERSON;
					}
					else if((int) koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_REF_TYPE] == DocuConstants.CODE_ZNACHENIE_REF_TYPE_FZL) {
						identifikatorString = "ЕГН"; // това остава - заради ССЕВ
						type = DocuConstants.SSEV_TYPE_PERSON;
					}
				...
				if(dopInfo != null && (int) koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_REF_TYPE] == DocuConstants.CODE_ZNACHENIE_REF_TYPE_FZL) {
				...
				predavane.setEmail((String) koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_CONTACT_EMAIL]);
				predavane.setDopInfo(dopInfo);
				predavane.setInformation(this.newPredavaneInfo.isEmpty() ? null : this.newPredavaneInfo.trim());
				predavane.setType((int) koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_REF_TYPE]);
				predavane.setMetodPredavane(this.newMetod);
				predavane.setDataPredavane(this.dataPredavane);
				predavane.setDataVrashtane((this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SEOS || this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV) ? null : this.dataVrashtane);
				predavane.setExemplar(this.newPredavaneEkz);
	...
	private void addNewDvijeniaSeos(List<Integer> codes) { //  TODO
		...
		try {
			for (int code : codes) {
				SystemClassif empl = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_EGOV_ORGANISATIONS, code, SysConstants.CODE_DEFAULT_LANG, date, true);
				if(empl == null) {
	...
	public void actionSelectAdmOrgan() {
		...
		try {
			...
			SystemClassif empl = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_EDELIVERY_ORGANISATIONS, code, SysConstants.CODE_DEFAULT_LANG, date, true);
			...
			predavane.setUchastnikType(DocuConstants.SSEV_TYPE_ADMINISTRATION);
	...
	public void actionSelectEgn() {
		...
		if(ValidationUtils.isValidEGN(this.newEgn)) {
			...
			predavane.setUchastnikType(DocuConstants.SSEV_TYPE_PERSON);
	...
	public void actionSelectEik() {
		...
		if(ValidationUtils.isValidBULSTAT(this.newEik)) {
			...
			predavane.setUchastnikType(DocuConstants.SSEV_TYPE_LEGALPERSON);
	...
	public void actionSelectZaOtgovor(int index) {
		...
		predavane.setUchastnikType(DocuConstants.SSEV_TYPE_REPLY);
	...
	private void addNewDvijenie(Dvijenie d, boolean askForEdit) {
		...
		if((this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL || this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_MAIL_CUSTOM)
			...
			// 
			if(getUserData().hasAccess(DocuConstants.CODE_CLASSIF_MENU, DocuConstants.CODE_ZNACHENIE_MENU_CORESP) && this.displayEditQuestion && askForEdit) {
		...
		if(this.newMetod == DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV && d.getUchastnikIdent() == null) {
		...
		else if(!this.newDvijenia.contains(d) && vavedenERachnoEmail && d.metodPredavane == DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL) {
			JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_INFO,
	...
	public List<SelectItem> getPredavaneTypeListNoEmailSystem() {
		return predavaneTypeList
				.stream()
				.filter(item -> (Integer)item.getValue() != DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL 
							&& (Integer)item.getValue()!= DocuConstants.CODE_ZNACHENIE_PREDAVANE_DRUGA_REGISTRATURA)
				.collect(Collectors.toList());
	}
	...
	public void setInformationText(String header, DocDvij dvijenie) {
		...
		try {
			if(dvijenie.getForRegid() != null && dvijenie.getStatus() == DocuConstants.DS_REGISTERED) {
				info += "<br>Регистратура: " + getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_REGISTRATURI, dvijenie.getForRegid(), SysConstants.CODE_DEFAULT_LANG, new Date(), false).getTekst();
			}
		}
		catch(DbErrorException e) {
			LOGGER.error(getMessageResourceString(beanMessages, ERROR_KLASSIF), e);
	...
	public String preparePostCoverNotice(int index) {		
		...
		if (null==forSend) {
			JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_ERROR, "Грешка при вземане на данни за лице!");
			return null;
		}
...
//						barcode.setAltText("Пощенски код");
	...
	public boolean viewRefModal(Integer idRefForModal) {
		...
		try {			
			...
			if(koresp != null) {
				bb = (int) koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_REF_TYPE] == DocuConstants.CODE_ZNACHENIE_REF_TYPE_NFL	||
				 	 (int) koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_REF_TYPE] == DocuConstants.CODE_ZNACHENIE_REF_TYPE_FZL;
					 //((int) koresp.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_REF_TYPE] == DocuConstants.CODE_ZNACHENIE_REF_TYPE_FZL && getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_SEE_PERSONAL_DATA));
			}
		} catch (DbErrorException e) {
			LOGGER.error(getMessageResourceString(beanMessages, ERROR_KLASSIF), e);
		}
