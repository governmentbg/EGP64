...
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_DELO_SECTION_INTERNAL;
   ...
   */
   ...
   private void loadDataFromDeloS(Object[] sDelo, Date inpDate) {	  
	   try {
		...
		if(Objects.equals(deloDocPrep.getDelo().getWithSection(),DocuConstants.CODE_ZNACHENIE_DA) &&
		...
		String msg =  deloDocPrep.getDelo().getRnDelo() +" / "+ DateUtils.printDate(deloDocPrep.getDelo().getDeloDate()) +
						"; "+ sd.decodeItem(DocuConstants.CODE_CLASSIF_DELO_STATUS, deloDocPrep.getDelo().getStatus(), getUserData().getCurrentLang(), new Date());
	...
	//	deloDocPrep.getDeloDelo().getDelo().setWithSection(SearchUtils.asInteger(sDelo[9]));  // при валгането на преписка в дело - няма раздели....
		...
		try {
			String msg =  deloDocPrep.getDeloDelo().getDelo().getRnDelo() +" / "+ DateUtils.printDate(deloDocPrep.getDeloDelo().getDelo().getDeloDate()) +
							"; "+  getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_DELO_STATUS, deloDocPrep.getDeloDelo().getDelo().getStatus(), getUserData().getCurrentLang(), new Date());
		...
		if(deloDocPrep != null && checkDataPrep()) { 			
			...
			try {
				...
				if (newEntries != null) { // ако има значи има новозаписани влагания, за които трябва да се прави проверка за движенията
					Integer s9 = sd.getRegistraturaSetting(this.docEdit.getRegistraturaId(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_9);
				...
				if(endDelo) {
					deloDocPrep.getDelo().setStatus(DocuConstants.CODE_ZNACHENIE_DELO_STATUS_COMPLETED);
				}
				if(endDeloDelo) {
					deloDocPrep.getDeloDelo().getDelo().setStatus(DocuConstants.CODE_ZNACHENIE_DELO_STATUS_COMPLETED);
				}
				endTask = false;
  ...
  private boolean checkDataPrepPrep(Date prepDate) {
   ...
   public void actionDelDeloLinkP(){
	   if (deloDocPrep.getDelo().getStatus()!=null &&
			(deloDocPrep.getDelo().getStatus()==DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_UA ||
			 deloDocPrep.getDelo().getStatus()==DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_DA)) {
	...
	public void actionEndDelo() {
		...
		if(endDelo) {
			try {
				...
				Integer s1 = sd.getRegistraturaSetting(docEdit.getRegistraturaId(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_10);
