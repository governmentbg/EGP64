...
public class DocDataVrazki extends IndexUIbean implements Serializable {
	...
	public void initVrazki() {
		...
		if (bean != null && !Objects.equals(bean.getRnFullDoc(), rnFullDoc)) { // значи за 1-ви път се отваря табчето
			...
			relType = DocuConstants.CODE_ZNACHENIE_DOC_REL_TYPE_VRAZKA;
			try {
				List<SystemClassif> items = getSystemData().getSysClassification( DocuConstants.CODE_CLASSIF_DOC_REL_TYPE, new Date(), getCurrentLang());
		...
		if (!selectedDoc.isEmpty())
			try {
				...
				for (Object[] obj : this.selectedDocs) {
					...
					JPA.getUtil().runInTransaction(() -> this.dao.save(null, relType, docId1, docId2));
				}
	...
	public void onRowEdit(RowEditEvent<Object> event) {
		...
		try {
			if (docId1.equals(row8)) { // права
				JPA.getUtil().runInTransaction(() -> this.dao.save(docDocId, relType, docId1, docId2));
			
			} else { // от другата страна се редактира
				JPA.getUtil().runInTransaction(() -> this.dao.save(docDocId, relType, docId2, docId1));
			}
			initDoc();
			JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_INFO,
