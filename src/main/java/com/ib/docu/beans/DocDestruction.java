...
public class DocDestruction extends IndexUIbean  implements Serializable {	
				...
				// отключване на всички обекти за потребителя(userId) и заключване на док., за да не може да се актуализира от друг				
					...
					if (idObj != null) {
						...
						JPA.getUtil().runWithClose(() -> {
							...
							this.filesListForWork = new FilesDAO(getUserData()).selectByFileObject(this.docProtDestruc.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
							
							this.filesListForOffic = new FilesDAO(getUserData()).selectByFileObject(this.docProtDestruc.getWorkOffId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
	...
	public void actionNew() {
		...
		this.docProtDestruc.setDocType(DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK);
		this.docProtDestruc.setFreeAccess(DocuConstants.CODE_ZNACHENIE_NE);
	...
	public void actionChangeAvtomNo() {
	   ...
	   public void actionChangeRegister() {
			...
			try {
				 if(this.docProtDestruc.getRegisterId() != null) {
					 alg  = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_REGISTRI, this.docProtDestruc.getRegisterId(), getUserData().getCurrentLang(), new Date(), DocuClassifAdapter.REGISTRI_INDEX_ALG);
				 }
