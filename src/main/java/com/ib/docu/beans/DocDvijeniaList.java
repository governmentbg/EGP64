...
import com.ib.docu.search.DocDvijSearch;
...
public class DocDvijeniaList extends IndexUIbean implements Serializable {
	...
	void initData() {
		...
		try {
			...
			docStatusList = createItemsList(false, DocuConstants.CODE_CLASSIF_DOC_STATUS, this.currentDate, true);
			...
			setRegistraturaList(createItemsList(false, DocuConstants.CODE_CLASSIF_REGISTRATURI, this.currentDate, false));
			
			//зарежда вида на предаване
			predavaneTypeList = // премахват се от списъка двете опции - имейл и сеос
					createItemsList(false, DocuConstants.CODE_CLASSIF_DVIJ_METHOD, currentDate, true);
			
//			for(SelectItem item: predavaneTypeList) {
//				if((Integer)item.getValue()== DocuConstants.CODE_ZNACHENIE_PREDAVANE_SEOS ||  (Integer)item.getValue()== DocuConstants.CODE_ZNACHENIE_PREDAVANE_SSEV 
...
//					.collect(Collectors.toList());
			...
			this.setWithEkzNum(Objects.equals(getSystemData().getSettingsValue("delo.docWithExemplars"),String.valueOf(DocuConstants.CODE_ZNACHENIE_DA)));
...
//		searchDocDvij.getDocTypeArr()[0] = DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN; 
//		searchDocDvij.getDocTypeArr()[1] = DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN;	
//		periodR = null;
//		periodRes = null;
//		periodAnswer = null;
//		periodAssign = null;
...
//			specificsRegister = Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_ALG,Optional.of(searchDocDvij.getRegistraturaId()));
//		}else {
