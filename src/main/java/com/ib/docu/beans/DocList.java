	...
	void initData() {
		...
		try {
			...
			//	docTypeList = createItemsList(false, DocuConstants.CODE_CLASSIF_DOC_TYPE, this.decodeDate, true);
				...
				docTypeList.add(new SelectItem(DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN, "входящ"));
				docTypeList.add(new SelectItem(DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN, "собствен"));
			}else {
				docTypeList = createItemsList(true, DocuConstants.CODE_CLASSIF_DOC_TYPE, this.decodeDate, true);
			...
			showRegList= isView && (getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_FULL_VIEW) ||
					getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_DOC_DELO_FULL_EDIT));
			setRegistraturaList(createItemsList(true, DocuConstants.CODE_CLASSIF_REGISTRATURI_OBJACCESS, this.decodeDate, false));
			clearFilter();
			this.setSettingMembers(((SystemData) getSystemData()).getRegistraturaSetting(getUserData(UserData.class).getRegistratura(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_7));
...
//				// ако не е показан избора за регистратура или е показан но е само един 
				specificsRegister = Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA, getUserData(UserData.class).getRegistratura());
//			}
		}else {
			Integer registratura = getUserData(UserData.class).getRegistratura();
			searchDoc = new DocSearch(registratura);
			specificsRegister = Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA, registratura);
		}
...
//		 }
	...
	public String actionGoto(int i, Object[] row) {
		String result = "";
		if(this.searchDoc != null && row != null) {
	...
	public void actionSelectCorresp() {
		...
		if(searchDoc.getCodeRefCorresp()!=null) {
			try {
				txtCorresp = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REFERENTS, searchDoc.getCodeRefCorresp(), getCurrentLang(), decodeDate);
	...
	public void actionChangeReg() {
		try {
			if(searchDoc.getRegistraturaId()!=null) {		
				specificsRegister = Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA,searchDoc.getRegistraturaId());
				
					this.setSettingMembers(((SystemData) getSystemData()).getRegistraturaSetting(searchDoc.getRegistraturaId(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_7));
