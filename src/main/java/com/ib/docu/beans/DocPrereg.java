...
public class DocPrereg extends IndexUIbean implements Serializable {
	...
	private boolean checkForLock(Integer idObj) {
		...
		try {
			Object[] obj = daoL.check(ud.getUserId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC, idObj);
			if (obj != null) {
				res = false;
	...
	public void lockDoc(Integer idObj) {
		...
		try {
			...
			JPA.getUtil().runInTransaction(
					() -> daoL.lock(ud.getUserId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC, idObj, null));
	...
	public void actionChangeDocVidPrereg() {
		try {
			...
			if (docPreregSettings != null && docPreregSettings[1] != null) {
				...
				Integer docTypeReg = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_REGISTRI,
						register, getUserData().getCurrentLang(), new Date(),
						DocuClassifAdapter.REGISTRI_INDEX_DOC_TYPE);
	...
	public void actionChangeRegister() {
		...
		try {
			...
			if (this.document.getRegisterId() != null) {
				alg = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_REGISTRI,
						document.getRegisterId(), getUserData().getCurrentLang(), new Date(),
						DocuClassifAdapter.REGISTRI_INDEX_ALG);
			}
		...
		if (checkDataDoc()) {
			...
			try {
				if (!avtomNo && (!Objects.equals(oldRn, document.getRnDoc()) ||
					...
					String errorRn = docDao.validateRnDoc(document, idDelo, sd, false, false);
					if (errorRn != null) {
