...
import com.ib.docu.db.dao.DocShemaDAO;
...
public class DocSchemaData extends IndexUIbean  implements Serializable {
	...
	public boolean checkData() {
		...
		if(docSchema.getPeriodType()==null  ) {
			...
			save = true;
		}else if(docSchema.getPeriodType().intValue() == DocuConstants.CODE_ZNACHENIE_SHEMA_PERIOD_DEFINED && (docSchema.getYears()==null || docSchema.getYears()<1)) {
	...
	public void actionSave() {
		...
		try {
			...
			getSystemData().reloadClassif(DocuConstants.CODE_CLASSIF_DOC_SHEMA, false, false);
