...
import com.ib.docu.db.dao.DocVidSettingDAO;
...
public class DocVidSettings extends IndexUIbean implements Serializable {	
	...
	public void initData() {
		...
		try {	
			...
			specificsIn.put(DocuClassifAdapter.PROCEDURI_INDEX_REGISTRATURA, this.idReg); // специфика за процедури по ид на регистратура
			specificsIn.put(DocuClassifAdapter.PROCEDURI_INDEX_STATUS, DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_ACTIVE); // специфика по статус активна за процедурите
			specificsIn.put(DocuClassifAdapter.PROCEDURI_INDEX_DOC_TYPE, DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN); // специфика по тип документ - входящ
			
			this.proceduriListDefIn = ((SystemData) getSystemData()).queryClassification(DocuConstants.CODE_CLASSIF_PROCEDURI, null, new Date(), getCurrentLang(), specificsIn);
			
			Map<Integer, Object> specificsWork = new HashMap<>();
			specificsWork.put(DocuClassifAdapter.PROCEDURI_INDEX_REGISTRATURA, this.idReg); // специфика за процедури по ид на регистратура
			specificsWork.put(DocuClassifAdapter.PROCEDURI_INDEX_STATUS, DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_ACTIVE); // специфика по статус активна за процедурите
			specificsWork.put(DocuClassifAdapter.PROCEDURI_INDEX_DOC_TYPE, DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK); // специфика по тип документ - работен
			
			this.proceduriListDefWork = ((SystemData) getSystemData()).queryClassification(DocuConstants.CODE_CLASSIF_PROCEDURI, null, new Date(), getCurrentLang(), specificsWork);
			
			Map<Integer, Object> specificsOwn = new HashMap<>();
			specificsOwn.put(DocuClassifAdapter.PROCEDURI_INDEX_REGISTRATURA, this.idReg); // специфика за процедури по ид на регистратура
			specificsOwn.put(DocuClassifAdapter.PROCEDURI_INDEX_STATUS, DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_ACTIVE); // специфика по статус активна за процедурите
			specificsOwn.put(DocuClassifAdapter.PROCEDURI_INDEX_DOC_TYPE, DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN); // специфика по тип документ - собствен
			
			this.proceduriListDefOwn = ((SystemData) getSystemData()).queryClassification(DocuConstants.CODE_CLASSIF_PROCEDURI, null, new Date(), getCurrentLang(), specificsOwn);
			
			this.settingMembers = ((SystemData) getSystemData()).getRegistraturaSetting(idReg, DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_7);
			
			this.docHarList = ((SystemData) getSystemData()).getSysClassification(DocuConstants.CODE_CLASSIF_CHARACTER_SPEC_DOC, new Date(), getCurrentLang());
	...
	public List<SelectItem> registriListByIdReg(Integer idRegistratura){
		...
		try {
			...
			String specific = (String) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_REGISTRATURI, idRegistratura, getCurrentLang(), new Date(), DocuClassifAdapter.REGISTRATURI_INDEX_REGISTRI);
			...
			for (String r : registersIds) {
				String name = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REGISTRI, Integer.valueOf(r), getCurrentLang(), new Date());
		...
		try {
			...
			if (idObj != null) {
				...
				JPA.getUtil().runWithClose(() -> {
					...
					this.filesList = new FilesDAO(getUserData()).selectByFileObject(this.dvSetting.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC_VID_SETT);
				});

				if (Objects.equals(this.dvSetting.getCreateDelo(), DocuConstants.CODE_ZNACHENIE_DA)) {
					this.createDelo = true;
				} else {
					this.createDelo = false;
				}
				
				if (Objects.equals(this.dvSetting.getMembersActive(), DocuConstants.CODE_ZNACHENIE_DA)) {
	...
	public boolean checkData() {
		...
		if(this.dvSetting.getBegNomer() == null && (this.registerAlg != null && this.registerAlg.equals(DocuConstants.CODE_ZNACHENIE_ALG_VID_DOC))) { 
		...
		if(this.dvSetting.getActNomer() == null && (this.registerAlg != null && this.registerAlg.equals(DocuConstants.CODE_ZNACHENIE_ALG_VID_DOC))) {
		...
		if(this.dvSetting.getStep() == null && (this.registerAlg != null && this.registerAlg.equals(DocuConstants.CODE_ZNACHENIE_ALG_VID_DOC))) {
	...
	public void actionSave() {
		...
		try {
			...
			if (this.createDelo) {
				this.dvSetting.setCreateDelo(DocuConstants.CODE_ZNACHENIE_DA);								
			} else {
				this.dvSetting.setCreateDelo(DocuConstants.CODE_ZNACHENIE_NE);	
			}
			
			if (this.memberActive) {
				this.dvSetting.setMembersActive(DocuConstants.CODE_ZNACHENIE_DA);								
			} else {
				this.dvSetting.setMembersActive(DocuConstants.CODE_ZNACHENIE_NE);	
			}
	...
	public void actionDelete() {
		...
		try {
			...
			JPA.getUtil().runInTransaction(() -> {
				...
				this.settDAO.delete(this.dvSetting);
	...
	public void actionChangeRegister() {
		...
		try {
			...
			if (this.dvSetting.getRegisterId() != null) {
				...
				this.registerAlg = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_REGISTRI, this.dvSetting.getRegisterId(), getCurrentLang(), new Date(), DocuClassifAdapter.REGISTRI_INDEX_ALG);
				
				if (this.registerAlg.equals(DocuConstants.CODE_ZNACHENIE_ALG_VID_DOC)) {
					if(!this.fromEdit) {
				...
				Integer registerTipDoc = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_REGISTRI, this.dvSetting.getRegisterId(), getCurrentLang(), new Date(), DocuClassifAdapter.REGISTRI_INDEX_DOC_TYPE);
				...
				if (registerTipDoc != null) {						
					...
					if (Objects.equals(registerTipDoc, DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN)) {
						this.viewProcDefIn = true;					
					
					} else if (Objects.equals(registerTipDoc, DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN)) {
						this.viewProcDefOwn = true;
						
					} else if (Objects.equals(registerTipDoc, DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK)) {
						this.viewProcDefWork = true;					
					}
	...
	public void actionChangeVidDoc() {
		...
		if (this.dvSetting.getDocVid() != null) {
			...
			try {
				...
				SystemClassif item = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_DOC_VID_SETTINGS, code, getCurrentLang(), new Date(), false);
				if (item != null) {
