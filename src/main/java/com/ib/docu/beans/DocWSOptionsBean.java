...
import com.ib.docu.db.dao.DocWSOptionsDAO;
	...
	public void actionEdit(Integer idObj) {
		...
		try {
			...
			if (idObj != null) {
				...
				if(Objects.equals(this.entity.getFreeAccess(), DocuConstants.CODE_ZNACHENIE_DA)) {
	...
	public void actionSave() {
		...
		try {
		 ...
		 if(this.limitedAccessCh) {
             this.entity.setFreeAccess(DocuConstants.CODE_ZNACHENIE_NE); //ограничен достъп
         } else {
             this.entity.setFreeAccess(DocuConstants.CODE_ZNACHENIE_DA); // свободен достъп 
         }
