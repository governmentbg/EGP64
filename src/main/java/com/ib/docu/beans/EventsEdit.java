...
import com.ib.docu.db.dao.EventDAO;
	...
	void initData() {
		...
		if ( SearchUtils.isEmpty(param)){
			...
			try {
				...
				if(event!=null) {
					JPA.getUtil().runWithClose( () -> this.filesList = filesDao.selectByFileObject(this.event.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_EVENT));		
					...
					if(event.getResourcesList() != null) {
						for( Integer item : event.getResourcesList()) {
							...
							SystemClassif scItem = new SystemClassif();
								...
								scItem.setCodeClassif(DocuConstants.CODE_CLASSIF_EVENT_RESOURCES);
								scItem.setCode(item);
								tekst = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_EVENT_RESOURCES, item, getUserData().getCurrentLang(), new Date());		
								scItem.setTekst(tekst);
			...
			try {
				...
				JPA.getUtil().runInTransaction(() -> { 
					for(Object[] item: selectedDocs) {
							...
							filesListDoc = filesDao.selectByFileObject(SearchUtils.asInteger(item[0]), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
							if(filesListDoc!=null && !filesListDoc.isEmpty()) {
								for(Files file:filesListDoc) {
									//filesList.add(file);							
									filesDao.saveFileObject(file, this.event.getId(),  DocuConstants.CODE_ZNACHENIE_JOURNAL_EVENT);
									flagSave = Boolean.TRUE;
								}
							}			
					}
					if(flagSave) {
						this.filesList = filesDao.selectByFileObject(this.event.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_EVENT);
						checkDateForNotif();
