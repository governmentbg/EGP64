...
//				String deloTypeTxt=(String) sd.decodeItem(DocuConstants.CODE_CLASSIF_DELO_TYPE, deloTypInt, lang, null);
...
//				System.out.println("registratura:"+registratura);
			...
			tempDocList=loadDoc(docId,zaTomId);
			tempDocList=mahameRabDoc(tempDocList);
			opisList.addAll(tempDocList);
	...
	private List<Object[]> mahameRabDoc(List<Object[]> tempDocList) {
		List<Object[]> tempList =new ArrayList<Object[]>();
		for (Object[] objects : tempDocList) {
			if ((objects[8].toString().equals("1")) && (objects[7].toString().equals("3"))) {
				continue;//преписка от тип "номенклатурно дело", т.е.DELO.DELO_TYPE=1, в описа да не се включват работни документи, т.е. с DOC.DOC_TYPE=3 .

			}
			tempList.add(objects);
		}
...
//			select.append(" DOC.COUNT_SHEETS ");//STAROTO
			...
			select.append(" , DOC.DOC_TYPE, delo.DELO_TYPE");//ot 18.03.2022 по искане на MA
...
/******************************** EXPORTS **********************************/
	...
	public Object[] dopInfoReport() {
		...
		try { 
			...
			int lang = getCurrentLang();
			deloTypeTxt=(String) sd.decodeItem(DocuConstants.CODE_CLASSIF_DELO_TYPE, deloTypInt, lang, null);
			//Четем датата на преписката
			 qq="select delo_date  from DELO WHERE DELO.DELO_ID=:DELO_ID";
