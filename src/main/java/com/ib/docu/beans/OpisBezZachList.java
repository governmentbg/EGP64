...
//		}
		...
		if (this.tipOpis == 2) {  // Опис за куриер
			 this.nachiniPred.add(Integer.valueOf(DocuConstants.CODE_ZNACHENIE_PREDAVANE_KURIER));    // Търсят се документи, предадени на ръка
			 this.nachiniPred.add(Integer.valueOf(DocuConstants.CODE_ZNACHENIE_PREDAVANE_POSHTA));    // И предадени по поща
		} else {	
		      this.nachiniPred.add(Integer.valueOf(DocuConstants.CODE_ZNACHENIE_PREDAVANE_NA_RAKA));    // Търсят се документи, предадени на ръка
		}      
...
//		setCheckUserAccess(getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_FULL_VIEW));
    ...
    public void actionSelectCorresp() {
		...
		if(getCodeRefCorresp()!=null) {
			try {
				txtCorresp = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REFERENTS, getCodeRefCorresp(), getCurrentLang(), decodeDate);
				setNameCorresp(null);
