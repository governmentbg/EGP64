...
@Named
@ViewScoped  
public class OpisZachList extends IndexUIbean implements Serializable  {
...
//		}
		...
		this.nachinPred = Integer.valueOf(DocuConstants.CODE_ZNACHENIE_PREDAVANE_NA_RAKA);
		...
		try {
			...
			docTypeList = createItemsList(false, DocuConstants.CODE_CLASSIF_DOC_TYPE, this.decodeDate, true);
			calendarList = createItemsList(false, DocuConstants.CODE_CLASSIF_PERIOD_NOFUTURE, this.decodeDate, true);
			
			setDisableRegList(getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_FULL_VIEW));
			List<SelectItem> regList = null;	
					 regList = createItemsList(false, DocuConstants.CODE_CLASSIF_REGISTRATURI_OBJACCESS, this.decodeDate, true);
	...
	public void clearFilter() {
		...
		specificsRegister = Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA, Optional.of(getUserData(UserData.class).getRegistratura()));
...
//		searchDoc.getDocTypeArr()[0] = DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN; 
//		searchDoc.getDocTypeArr()[1] = DocuConstants.CODE_ZNACHENIE_DOC_TYPE_OWN;	
		periodR = null;
		periodRes = null;
		periodAnswer = null;
		periodAssign = null;
   ...
   public void actionChangeReg() {
		...
		if(searchDoc.getRegistraturaId()!=null) {		
			specificsRegister = Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA,Optional.of(searchDoc.getRegistraturaId()));
		}else {
    ...
    */
	public void actionSelectCorresp() {
		...
		if(searchDoc.getCodeRefCorresp()!=null) {
			try {
				txtCorrespDoc = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REFERENTS, searchDoc.getCodeRefCorresp(), getCurrentLang(), decodeDate);
...
//	
    ...
    public void actionSelectCorrespPred() {
		...
		if(getCodeRefCorrespPred()!=null) {
			try {
				txtCorrespPred = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REFERENTS, getCodeRefCorrespPred(), getCurrentLang(), decodeDate);
...
//				tmpSc.setCodeClassif(DocuConstants.CODE_CLASSIF_REFERENTS);
//				tmpSc.setCode(codeRefCorrSrch);
//				tekst = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REFERENTS, codeRefCorrSrch, getUserData().getCurrentLang(), new Date());
	...
	// Управление на модален екран за търсене на предадени документи
			...
			public void changeDocType() {
				...
				for(Integer item:searchDoc.getDocTypeArr()) {
					if(item!=null && item.intValue()==DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK) {
...
//		this.docSelectedTmp.clear();
	...
	List <Object[]> addTekstsForClass (List<Object[]> objList) {
		...
		for (Object[] obj : objList) {
			...
			try {
				obj1[obj.length] = (String)getSystemData().decodeItem(Integer.valueOf(DocuConstants.CODE_CLASSIF_DOC_TYPE), asInteger(obj[6]), getCurrentLang(), new Date());
			} catch (DbErrorException e) {
				//  Auto-generated catch block
			...
			try {
				obj1[obj.length+1] = (String)getSystemData().decodeItem(Integer.valueOf(DocuConstants.CODE_CLASSIF_DOC_VID), asInteger(obj[7]), getCurrentLang(), new Date());
			} catch (DbErrorException e) {
				//  Auto-generated catch block
			         ...
			         //     this.dvijDoc.setEkzNomer(Integer.valueOf((String)obj[12]));
		                ...
		                try {
		        			...
		        			if(isAdmStr(asInteger(obj[10]))) {
		        				SystemClassif empl = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_ADMIN_STR, asInteger(obj[10]), SysConstants.CODE_DEFAULT_LANG, new Date(), true);
		        				if(empl != null) {
		        					if (empl.getSpecifics() != null)
		        					  email = (String) empl.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_CONTACT_EMAIL];
		        				}
		        			}
		        				...
		        				if(empl != null) {
		        					if (empl.getSpecifics() != null)
		        					  email = (String) empl.getSpecifics()[DocuClassifAdapter.REFERENTS_INDEX_CONTACT_EMAIL];
		        				}
		        			}
...
//		        			JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_ERROR, e.getMessage());	
							 ...
							 if (docType.intValue() == DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN  && docProcess.intValue() == DocuConstants.CODE_ZNACHENIE_NE) {
								 ...
								 Query q = JPA.getUtil().getEntityManager().createNativeQuery("UPDATE DOC SET PROCESSED = :DA  WHERE DOC_ID = :IDD");
									q.setParameter("DA",  DocuConstants.CODE_ZNACHENIE_DA);
									q.setParameter("IDD", docRegId);
									q.executeUpdate();
									
									String docIdent = obj[2] + "/" + DateUtils.printDate((Date) obj[5]);								 
									SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC, docRegId
									...
									dao.saveAudit(journal);
									 ...
									 docProcess = Integer.valueOf(DocuConstants.CODE_ZNACHENIE_DA);
	...
	public boolean isAdmStr(Integer codeRef) {
		if(codeRef == null) return false;
			...
			try {
				return getSystemData().matchClassifItems(DocuConstants.CODE_CLASSIF_ADMIN_STR_REPORTS, codeRef, new Date());
			} catch (DbErrorException e) {
//				LOGGER.error(getMessageResourceString(beanMessages, ERROR_KLASSIF), e);
//				JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_ERROR, e.getMessage());	
				return false;
