...
import com.ib.docu.print.RegistriDocsPrint;
...
//		this.periodR = 9; // този месец	
	...
	public String exportOpis() {
		...
		if (JSFUtils.getRequestParameter("expT") != null && !"".equals(JSFUtils.getRequestParameter("expT"))) {
			...
			expType=DocuConstants.EXPORT_HTML;
		}
...
/******************************** EXPORTS **********************************/
	...
	public void getReportTitles() {
		...
		try {
			...
			if (null!=searchDoc.getRegisterId())
				this.dopInfo[0]=getMessageResourceString(LABELS, "docu.register")+": "+ getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REGISTRI, searchDoc.getRegisterId(), currentLang, new Date());
