...
public class ProcDefEdit extends IndexUIbean  implements Serializable {	
			...
			if (JSFUtils.getRequestParameter(ID_OBJ) != null && !"".equals(JSFUtils.getRequestParameter(ID_OBJ))) {
				...
				if (idObj != null) {
					...
					JPA.getUtil().runWithClose(() -> {
						...
						this.filesList = new FilesDAO(getUserData()).selectByFileObject(this.procDef.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_PROC_DEF);
				...
				this.procDef.setRegistraturaId(getUserData(UserData.class).getRegistratura());
				this.procDef.setStatus(DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_DEV );
				this.procDef.setWorkDaysOnly(DocuConstants.CODE_ZNACHENIE_NE);
				this.oldStatus = this.procDef.getStatus();
			}			
	...
	public void actionSelectSlujForProc() {
		...
		try {
			...
			if (this.procDef.getCodeRef() != null) {
				...
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.procDef.getCodeRef(), getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void actionSelectZvenoForProc() {
		...
		try {
			...
			if (this.procDef.getZveno() != null) {
				...
				// проверява се типа дали е служител, за да не се избере звено, което няма служители
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.procDef.getZveno(), getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public String actionGotoDiagram() {
		...
		return "testDiagram.jsf?faces-redirect=true&idProc=" + this.procDef.getId();
	}
