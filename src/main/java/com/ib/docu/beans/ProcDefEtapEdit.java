...
import com.ib.docu.db.dao.ProcDefEtapDAO;
import com.ib.docu.db.dao.ProcDefTaskDAO;
...
public class ProcDefEtapEdit extends IndexUIbean  implements Serializable {	
	...
	public void initData() {
		...
		try {
			...
			List<SystemClassif> itemsEmplPos = getSystemData().getSysClassification(DocuConstants.CODE_CLASSIF_POSITION, new Date(), getCurrentLang());
			List<SystemClassif> itemsBusRole = getSystemData().getSysClassification(DocuConstants.CODE_CLASSIF_PROC_BUSINESS_ROLE, new Date(), getCurrentLang());
			...
			if (proc.getWorkDaysOnly().equals(SearchUtils.asInteger(DocuConstants.CODE_ZNACHENIE_DA))){
	...
	public void actionNewEtap() {			
		...
		this.defEtap.setIsMerge(DocuConstants.CODE_ZNACHENIE_NE);
	...
	private boolean checkDataForEtap() {
		...
		if (this.defEtap.getNomer() == null) {
			JSFUtils.addMessage(FORM_PROC_DEF_ETAP + ":etapNomer", FacesMessage.SEVERITY_ERROR, getMessageResourceString (UI_beanMessages, MSGPLSINS, getMessageResourceString(LABELS, "procDefEtapEdit.nomerEtap")));
			save = true;
		...
		if(SearchUtils.isEmpty(this.defEtap.getEtapName())) {
			JSFUtils.addMessage(FORM_PROC_DEF_ETAP + ":etapName", FacesMessage.SEVERITY_ERROR, getMessageResourceString (UI_beanMessages, MSGPLSINS, getMessageResourceString(LABELS, "procDefEtapEdit.nameEtap")));
			save = true;
		}
...
//		}
		...
		if(SearchUtils.isEmpty(this.defEtap.getEtapInfo())) {
			JSFUtils.addMessage(FORM_PROC_DEF_ETAP + ":etapInfo", FacesMessage.SEVERITY_ERROR, getMessageResourceString (UI_beanMessages, MSGPLSINS, getMessageResourceString(LABELS, "procDefEtapEdit.opisEtap")));
			save = true;
		}
	...
	public void actionSelectSlujForEtap() {
		...
		try {
			...
			if (this.defEtap.getCodeRef() != null) {
				...
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.defEtap.getCodeRef(), getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void actionSelectZvenoForEtap() {
		...
		try {
			...
			if (this.defEtap.getZveno() != null) {
				...
				// проверява се типа дали е служител, за да не се избере звено, което няма служители
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.defEtap.getZveno(), getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void actionNewTask() {	
		...
		this.defTask.setIndividual(DocuConstants.CODE_ZNACHENIE_NE);
		this.defTask.setDocRequired(DocuConstants.CODE_ZNACHENIE_NE);
		...
		try {
			...
			if(this.defTask.getTaskIzpList() != null) {
				for (int i = 0; i < this.defTask.getTaskIzpList().size(); i++) {
					...
					if (i == 0) {
						taskIzp.setIzpRole(DocuConstants.CODE_ZNACHENIE_TASK_REF_ROLE_EXEC_OTG);
					} else {
						taskIzp.setIzpRole(DocuConstants.CODE_ZNACHENIE_TASK_REF_ROLE_EXEC);
					}
				}				
			}
	...
	public void actionDeleteTask() {
		...
		if (this.proc.getStatus().intValue() != DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_DEV && this.defTasksList.size() == 1) {
	...
	public void actionSelectSlujForTaskVazl() {
		...
		try {
			...
			if (this.defTask.getAssignCodeRef() != null) {
				...
				// проверява се типа дали е служител, за да не се избере звено, което няма служители
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.defTask.getAssignCodeRef(), getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void actionSelectZvenoForTaskVazl() {
		...
		try {
			...
			if (this.defTask.getAssignZveno() != null) {
				...
				// проверява се типа дали е служител, за да не се избере звено, което няма служители
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.defTask.getAssignZveno(), getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void actionSelectSlujForTaskIzp(Integer codeRef) {
		...
		try {
			...
			if (codeRef != null) {
				...
				// проверява се типа дали е служител, за да не се избере звено, което няма служители
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, codeRef, getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void actionSelectZvenoForTaskIzp(Integer zveno) {
		...
		try {
			...
			if (zveno != null) {
				...
				// проверява се типа дали е служител, за да не се избере звено, което няма служители
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, zveno, getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void actionChangeTaskType() {
		...
		try {
			...
			List<SystemClassif> tmpOpinionList = getSystemData().getClassifByListVod(DocuConstants.CODE_LIST_TASK_TYPE_TASK_OPINION, this.defTask.getTaskType(), getCurrentLang(), new Date());
			
			if (tmpOpinionList != null && !tmpOpinionList.isEmpty()) {
				this.showOpinion = true;	
				
				for (SystemClassif tmpSc : tmpOpinionList) {
					if (tmpSc.getCode() != DocuConstants.CODE_ZNACHENIE_TASK_OPINION_STOP_PROC) {
			...
			if (this.defTask.getOpinionOkList() != null && !this.defTask.getOpinionOkList().isEmpty()) {					
				for (Integer ok : this.defTask.getOpinionOkList()) {
					SystemClassif tmp = new SystemClassif();						
					tmp = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_TASK_OPINION, ok, getCurrentLang(), new Date(), false);
			...
			if (this.defTask.getOpinionNotList() != null && !this.defTask.getOpinionNotList().isEmpty()) {					
				for (Integer not : this.defTask.getOpinionNotList()) {
					SystemClassif tmp = new SystemClassif();						
					tmp = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_TASK_OPINION, not, getCurrentLang(), new Date(), false);
			...
			if (this.defTask.getOpinionOptionalList() != null && !this.defTask.getOpinionOptionalList().isEmpty()) {					
				for (Integer opt : this.defTask.getOpinionOptionalList()) {
					SystemClassif tmp = new SystemClassif();						
					tmp = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_TASK_OPINION, opt, getCurrentLang(), new Date(), false);
