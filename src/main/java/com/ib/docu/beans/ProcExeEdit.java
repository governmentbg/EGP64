				...
				// отключване на всички обекти за потребителя(userId) и заключване на процедура, за да не може да се актуализира от друг			
					...
					if (idObj != null) {
						...
						JPA.getUtil().runWithClose(() -> {
							...
							this.filesListForDefProc = new FilesDAO(getUserData()).selectByFileObject(this.procDef.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_PROC_DEF);
							
							if (this.procExe.getStatus() == DocuConstants.CODE_ZNACHENIE_PROC_STAT_WAIT || this.procExe.getStatus() == DocuConstants.CODE_ZNACHENIE_PROC_STAT_EXE 								
							...
							if (this.procExe.getStatus() == DocuConstants.CODE_ZNACHENIE_PROC_STAT_STOP && this.procDef.getStatus() == DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_ACTIVE								
									&& (this.procExe.getCodeRef() != null && getUserData().getUserId().equals(this.procExe.getCodeRef())
									 || getUserData().hasAccess(DocuConstants.CODE_CLASSIF_BUSINESS_ROLE, DocuConstants.CODE_ZNACHENIE_BUSINESS_ROLE_PROC_ADMIN)
									 || getUserData(UserData.class).getUserAccess().equals(this.procExe.getCodeRef())
									 || this.procExeDAO.checkEtapControlUser(this.procExe.getId(), this.procExe.getStatus()))) {
		...
		if (this.selEtapList.isEmpty()) {			
			...
			if (this.selEtapList.size() > 1) {
				...
				for (Object[] obj : selEtapList) {
					...
					if (SearchUtils.asInteger(obj[11]).equals(DocuConstants.CODE_ZNACHENIE_ETAP_STAT_IZP) || SearchUtils.asInteger(obj[11]).equals(DocuConstants.CODE_ZNACHENIE_ETAP_STAT_IZP_SROK)) {
...
/******************************************************* LOCK & UNLOCK *******************************************************/	
	...
	public Map<Long, String> getColorStatus() {
		...
		if (this.colorStatus == null) {
			...
			this.colorStatus.put(SearchUtils.asLong(DocuConstants.CODE_ZNACHENIE_ETAP_STAT_WAIT), "color:#F28686;");
			this.colorStatus.put(SearchUtils.asLong(DocuConstants.CODE_ZNACHENIE_ETAP_STAT_DECISION), "color:#F28686;");
			this.colorStatus.put(SearchUtils.asLong(DocuConstants.CODE_ZNACHENIE_ETAP_STAT_EXE), "color:#99CE6B;");
			this.colorStatus.put(SearchUtils.asLong(DocuConstants.CODE_ZNACHENIE_ETAP_STAT_IZP), "color:#50A2B9;");
			this.colorStatus.put(SearchUtils.asLong(DocuConstants.CODE_ZNACHENIE_ETAP_STAT_IZP_SROK), "color:#50A2B9;");
		}
