	...
	public void actionFindAllDefProcedures(boolean createProc) {
		...
		try {
			...
			if (createProc) {
				...
				specifics.put(DocuClassifAdapter.PROCEDURI_INDEX_REGISTRATURA, getUserData(UserData.class).getRegistratura()); // специфика за процедури по ид на регистратура
				specifics.put(DocuClassifAdapter.PROCEDURI_INDEX_STATUS, DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_ACTIVE); // специфика по статус активна за процедурите
				specifics.put(DocuClassifAdapter.PROCEDURI_INDEX_DOC_TYPE, 0); // специфика по тип инициращ документ
		
				this.proceduriList = getSystemData().queryClassification(DocuConstants.CODE_CLASSIF_PROCEDURI, null, this.decodeDate, getCurrentLang(), specifics);
				...
				Map<Integer, Object> specifics = new HashMap<>();
				specifics.put(DocuClassifAdapter.PROCEDURI_INDEX_REGISTRATURA, this.idReg); // специфика за процедури по ид на регистратура
				specifics.put(DocuClassifAdapter.PROCEDURI_INDEX_STATUS, DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_ACTIVE); // специфика по статус активна за процедурите
		
				this.defProcList = getSystemData().queryClassification(DocuConstants.CODE_CLASSIF_PROCEDURI, null, this.decodeDate, getCurrentLang(), specifics);
			}
