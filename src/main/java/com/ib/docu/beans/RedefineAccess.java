	...
	public void actionSelectSluj() {
		...
		try {
			...
			for (SystemClassif tmpSluj : scReferentsList) {
				...
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, tmpSluj.getCode(), getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);

				if (!existSluj && refType.equals(Integer.valueOf(DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL))) {
			...
			if (!slujList.isEmpty()) {
				...
				for (Object[] sluj : slujList) {				
					...
					if (SearchUtils.asString(sluj[3]).isEmpty()) { // ако не е потребител - няма да проверявам дали е логнат в системата и се добавя за преопределяне на достъп
						...
						SystemClassif sc = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_ADMIN_STR, SearchUtils.asInteger(sluj[0]), getCurrentLang(), decodeDate, false);
