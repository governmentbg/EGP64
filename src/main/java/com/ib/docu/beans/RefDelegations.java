...
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_DELEGATES;
...
public class RefDelegations extends IndexUIbean  implements Serializable {
	...
	void initData() {
		...
		try {
			...
			specifics = Collections.singletonMap(DocuClassifAdapter.USERS_INDEX_REGISTRATURA,registratura);
			//specificsAdmStr.put( DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA, registratura);
			//specificsAdmStr.put( DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE , DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL);
			refDao = new ReferentDelegationDAO(getUserData());
			liceNames = getUserData().getLiceNames();
			showAutoriz  = getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_AUTHORIZE);
			delegRight = getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA,DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_DELEGATE_FOREIGN);
	...
	public void actionNewSubst() {
		...
		delegRule.setDelegationType(DocuConstants.CODE_ZNACHENIE_DELEGATES_ZAMESTVANE );
	}
	public void actionNewAuthorization() {
		...
		delegRule.setDelegationType(DocuConstants.CODE_ZNACHENIE_DELEGATES_UPYLNOMOSHTAVANE );
...
//		delegRule.setDateTo(cal.getTime());
	...
	public boolean checkData() {
		...
		if(delegRule.getDateTo()==null && delegRule.getDelegationType().intValue() == DocuConstants.CODE_ZNACHENIE_DELEGATES_ZAMESTVANE ) {
	...
	public void actionSave() {
		...
		try {
			...
			getSystemData().reloadClassif(DocuConstants.CODE_CLASSIF_EMPL_REPLACES, false, false);
	...
	public void actionEdit(Integer id) {
		...
		if(delegRule!=null) {
			if(delegRule.getDelegationType().intValue() == DocuConstants.CODE_ZNACHENIE_DELEGATES_ZAMESTVANE) {
				replaced=true;
			}else if(delegRule.getDelegationType().intValue() == DocuConstants.CODE_ZNACHENIE_DELEGATES_UPYLNOMOSHTAVANE) {
				replaced = false; 
			}
		}	
	}
