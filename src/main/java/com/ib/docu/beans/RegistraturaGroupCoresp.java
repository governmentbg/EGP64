...
import com.ib.docu.db.dao.RegistraturaGroupDAO;
...
public class RegistraturaGroupCoresp extends IndexUIbean implements Serializable {
	...
	public void initData() {
		...
		try {
			...
			if (getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_REG_GROUP_ADM)) {	
				
				this.registraturiList = createItemsList(false, DocuConstants.CODE_CLASSIF_REGISTRATURI, new Date(), false);
				...
				if (getUserData(UserData.class).isDelovoditel()) {
					...
					this.registraturiList = createItemsList(true, DocuConstants.CODE_CLASSIF_REGISTRATURI, new Date(), true);
					...
					String nameReg = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REGISTRATURI, getUserData(UserData.class).getRegistratura(), getCurrentLang(), new Date());
			...
			List<SystemClassif> itemsGrType = getSystemData().getSysClassification(DocuConstants.CODE_CLASSIF_REGISTRATURA_GROUP_TYPE, new Date(), getCurrentLang());
			
			for (SystemClassif item : itemsGrType) {
				if (item.getCode() == DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP || item.getCode() == DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS) {
	...
	public void actionSearch(){
		...
		try {
			...
			if (this.grType != null) {				
				...
				groupTypeList.add(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP);
				groupTypeList.add(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS);
			}
				...
				if(this.groupCoresp.getReferentIds() != null && !this.groupCoresp.getReferentIds().isEmpty()) {
					...
					for (Integer item : this.groupCoresp.getReferentIds()) {
						...
						try {
							...
							scItem.setCodeClassif(DocuConstants.CODE_CLASSIF_EGOV_ORGANISATIONS);
							scItem.setCode(item);
							tekst = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_EGOV_ORGANISATIONS, item, getUserData().getCurrentLang(), new Date());		
	...
	public void actionNew() {
		...
		this.groupCoresp = new RegistraturaGroup();
		this.groupCoresp.setGroupType(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP); 
	...
	public void actionChangeGroupType() {
		...
		if (typeGroup.equals(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP)) {
			this.groupCoresp.setGroupType(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP);			
		
		} else if (typeGroup.equals(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS)) {
			this.groupCoresp.setGroupType(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS);			
		}		
	...
	public void actionSearch(Integer groupId) {
		...
		if (this.groupCoresp.getGroupType().equals(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP)) {
			this.correspList = new LazyDataModelSQL2Array(smd, defaultSortColumn);		
		
		} else if (this.groupCoresp.getGroupType().equals(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS)) {		
...
//			this.groupCoresp.setGroupType(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP);
			...
			getSystemData().reloadClassif(DocuConstants.CODE_CLASSIF_GROUP_CORRESP, false, false);
...
//			actionNew();
			...
			JPA.getUtil().runWithClose(() -> this.regGrCorespList = this.regGrDAO.findByRegistraturaId(this.usersRegList, Arrays.asList(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP, DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS), codeRef, name, info));
