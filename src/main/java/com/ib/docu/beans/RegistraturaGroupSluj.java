...
//		this.specificsReferents  = Collections.singletonMap(DocuClassifAdapter.REFERENTS_INDEX_REGISTRATURA, this.idReg); 
	...
	public void actionSearch(){
		...
		try {
			...
			JPA.getUtil().runWithClose(() -> this.regGrSlujList = this.regGrDAO.findByRegistraturaId(this.usersRegList, Arrays.asList(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_EMPL), this.codeRef, this.name, this.info));
				...
				if(this.groupSluj.getReferentIds() != null && !this.groupSluj.getReferentIds().isEmpty()) {
					...
					for (Integer item : this.groupSluj.getReferentIds()) {
						...
						try {
							...
							scItem.setCode(item);
							tekst = getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_ADMIN_STR, item, getUserData().getCurrentLang(), new Date());		
	...
	public void actionSearch(Integer groupId) {
		...
		SelectMetadata smd = this.regGrDAO.findReferentsByIdGroup(groupId, DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_EMPL, getUserData());
		...
		if (this.groupSluj.getReferentIds() != null) {
			...
			try {
				...
				for (Integer refCode : this.groupSluj.getReferentIds()) {
					...
					Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, refCode, getCurrentLang(), new Date(), DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
				...
				getSystemData().reloadClassif(DocuConstants.CODE_CLASSIF_GROUP_EMPL, false, false);
...
//		}
	...
	public void actionSave() {
		...
		try {
			...
			this.groupSluj.setGroupType(DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_EMPL);
