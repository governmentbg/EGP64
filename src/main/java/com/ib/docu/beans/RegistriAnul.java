...
public class RegistriAnul extends IndexUIbean  implements Serializable {	
	...
	void initData() {
		...
		this.tipAlg=DocuConstants.CODE_ZNACHENIE_ALG_INDEX_STEP;
	...
	public void actionLoadSelected() {
		...
		try {
			...
			if (null!=this.selRegistratura && null!=this.getTipAlg()) {									
				...
				if (this.tipAlg.equals(Integer.valueOf(DocuConstants.CODE_ZNACHENIE_ALG_INDEX_STEP))) {// Algoritam index with steps
	...
	public void actionAnulSelRegisters() {
		...
		if (this.tipAlg.equals(Integer.valueOf(DocuConstants.CODE_ZNACHENIE_ALG_INDEX_STEP))){ //1. Registri za documenti s alg index+step
	...
	public Object[] getReportTitles() {
		...
		try {
			...
			if (null!=selRegistratura)
				dopInfoT[1]=getMessageResourceString(LABELS, "regData.registratura")+": "+ getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_REGISTRATURI, selRegistratura, currentLang, new Date());
			
			if (null!=tipAlg) {
				
				if (tipAlg.equals(Integer.valueOf(DocuConstants.CODE_ZNACHENIE_ALG_INDEX_STEP))){
