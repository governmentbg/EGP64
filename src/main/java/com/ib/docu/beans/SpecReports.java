...
private static final Logger LOGGER = LoggerFactory.getLogger(SpecReports.class);
	...
	void initData() {
		try {
			sprList = getSystemData().getSysClassification(DocuConstants.CODE_CLASSIF_RPT_SPEC_ACTIVE, new Date (),getUserData().getCurrentLang());
			if(sprList!=null && !sprList.isEmpty()) {
				
				Iterator<SystemClassif> i = sprList.iterator();
				while (i.hasNext()) {
				SystemClassif s = i.next(); 
					if(!getUserData().hasAccess(DocuConstants.CODE_CLASSIF_RPT_SPEC_ACTIVE, s.getCode())) {
						  i.remove();
					}	  
	...
	public String actionGoTo(Integer code) {
		...
		String link = "";
		try {
			link = getSystemData().decodeItemDopInfo(DocuConstants.CODE_CLASSIF_RPT_SPEC , code, getUserData().getCurrentLang(), new Date());
		} catch (DbErrorException e) {
