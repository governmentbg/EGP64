...
public class TakeAwayAccess  extends IndexUIbean   {
	...
	public void initData() {
		...
		setSpecificsRegisterSource(Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA, Optional.of(this.userRegistratura)));	
	}
	...
	public void actionSelectSlujForTakeAway() {
		...
		try {
			...
			if (this.codeTakeAwayAccess != null) {
				...
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.codeTakeAwayAccess, getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);

				if (refType.equals(Integer.valueOf(DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL))) {
			
					this.userLeave = getSystemData().matchClassifItems(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.codeTakeAwayAccess, this.decodeDate);
					...
					SystemClassif sc = getSystemData().decodeItemLite(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.codeTakeAwayAccess, getCurrentLang(), decodeDate, true);
					...
					Integer position = SearchUtils.asInteger(sc.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_POSITION]);
					if (position != null) {
						
						boolean boss = getSystemData().matchClassifItems(DocuConstants.CODE_CLASSIF_BOSS_POSITIONS, position, this.decodeDate);
						
						if (boss) {
							
							// трябват ми само звената и за това пускам през специфика да се отрежат
							Map<Integer, Object> specifics = Collections.singletonMap(DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE, DocuConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO);

							List<SystemClassif> items = getSystemData().queryClassification(DocuConstants.CODE_CLASSIF_ADMIN_STR, null, this.decodeDate, getCurrentLang(), specifics);
