...
import com.ib.docu.db.dao.TaskScheduleDAO;
...
public class TaskScheduleBean extends IndexUIbean  implements Serializable {
	...
	void initData() {
		...
		specificsAdm = Collections.singletonMap(DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE
				, X.of(DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL));
		specificsAdmZveno = Collections.singletonMap(DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE
				, X.of(DocuConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO)); 
		...
		if ( SearchUtils.isEmpty(param)){
			...
			try {
				...
				if(task.getValid()==null || task.getValid().intValue() == DocuConstants.CODE_ZNACHENIE_NE) {
					valid =false;
				}else {
					valid = true;
				}
				
				if(task.getWorkDaysOnly()==null || task.getWorkDaysOnly().intValue() == DocuConstants.CODE_ZNACHENIE_NE) {
					workDays =false;
				}else {
					workDays = true;
				}
				
				if(task.getDocRequired()==null || task.getDocRequired().intValue() == DocuConstants.CODE_ZNACHENIE_NE) {
					docFinish =false;
				}else {
					docFinish = true;
				}
	...
	private boolean checkData() {
		...
		if(task.getRegPeriod()==null) {
			...
			if(task.getRegPeriod()==DocuConstants.CODE_ZNACHENIE_TASK_PERIOD_DAY) {
				task.setRegInterval(1);
			}else if(task.getRegPeriod()==DocuConstants.CODE_ZNACHENIE_TASK_PERIOD_WEEK ||task.getRegPeriod()==DocuConstants.CODE_ZNACHENIE_TASK_PERIOD_MONTH
					||task.getRegPeriod()==DocuConstants.CODE_ZNACHENIE_TASK_PERIOD_YEAR) {
				...
				if(task.getRegDay()==null) {
					if(task.getRegPeriod()==DocuConstants.CODE_ZNACHENIE_TASK_PERIOD_WEEK) {
						...
						flagSave = true;
					}else if(task.getRegPeriod()==DocuConstants.CODE_ZNACHENIE_TASK_PERIOD_MONTH ||task.getRegPeriod()==DocuConstants.CODE_ZNACHENIE_TASK_PERIOD_YEAR) {						
			...
			if(task.getRegPeriod()==DocuConstants.CODE_ZNACHENIE_TASK_PERIOD_YEAR && task.getRegMonth()==null) {
...
//		}
	...
	public void actionChangeZveno() {
		if(task.getZveno()==null) {
			...
			try {							
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, task.getZveno(), getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
