...
public class TasksList  extends IndexUIbean{
	...
	public void initData() {
		...
		try {
			vidTask = createItemsList(false, DocuConstants.CODE_CLASSIF_TASK_VID, taskSearch.getRegDateTo() == null ? new Date() : taskSearch.getRegDateTo(), true);
			statusTaskList = createItemsList(false, DocuConstants.CODE_CLASSIF_TASK_STATUS, taskSearch.getRegDateTo() == null ? new Date() : taskSearch.getRegDateTo(), true);
			endOpinionTaskList = createItemsList(false, DocuConstants.CODE_CLASSIF_TASK_OPINION, taskSearch.getRegDateTo() == null ? new Date() : taskSearch.getRegDateTo(), true);
			vidSlujList = createItemsList(false, DocuConstants.CODE_CLASSIF_TASK_REF_ROLE , new Date(), true);
			showRegistaturiList = getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_FULL_VIEW);
			showGrupoviDeinosti = getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_GROUP_TASK);
			showGroupDeleteBtn  = getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_DELETE_TASK);

			registraturaList = createItemsList(true, DocuConstants.CODE_CLASSIF_REGISTRATURI_OBJACCESS , new Date(), false);
			...
			Integer s1 = ((SystemData) getSystemData()).getRegistraturaSetting(getUserData(UserData.class).getRegistratura(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_15);
		...
		hasAccessTaskRegis = getUserData().hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_TASK_FULL_EDIT);
	}
    ...
    public void grupovoSnemaneTasks() {
    	...
    	for (int i = 0; i < taskSelectedAllM.size(); i++) {
			try {
				...
				task.setStatus(DocuConstants.CODE_ZNACHENIE_TASK_STATUS_SNETA);
    ...
    public void showModalIzpTasksModal() {
		...
		try {
	    	...
	    	List<SystemClassif> tmpOpinionLst = getSystemData().getClassifByListVod(DocuConstants.CODE_LIST_TASK_TYPE_TASK_OPINION, Integer.valueOf(tmp[2].toString()), getCurrentLang(),  new Date());
    	...
    	for (int i = 0; i < taskSelectedAllM.size(); i++) {
			try {
		    	...
		    	if(tmpOpinionLst == null || tmpOpinionLst.isEmpty()) {
					...
					task.setStatus(DocuConstants.CODE_ZNACHENIE_TASK_STATUS_IZP);
					task.setRealEnd(dateGrupovoSnemane);
					task.setStatusComments(textGrupovoSnemane);
					if(showOpinionList)
	...
	public String titleLock(Integer from, Date dateL) {
		...
		if(from != null && dateL != null) {
			...
			try {
				sb.append(getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_ADMIN_STR, from , getCurrentLang(), new Date()));
