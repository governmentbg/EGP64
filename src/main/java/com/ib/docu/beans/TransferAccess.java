	...
	public void actionTransferAccess() {
		...
		if (transfer) {
			...
			try {			
				...
				Integer regIdUserForTake = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.codeTakeAccess, getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA);
				Integer regIdUserForGive = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.codeGiveAccess, getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA);
	...
	public void actionSelectSlujForTake() {
		...
		try {
			...
			if (this.codeTakeAccess != null) {
				...
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.codeTakeAccess, getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void actionSelectSlujForGive() {
		...
		try {
			...
			if (this.codeGiveAccess != null) {
				...
				// проверява се типа дали е служител, за да не се избере звено, което няма служители
				Integer refType = (Integer) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_ADMIN_STR, this.codeGiveAccess, getCurrentLang(), this.decodeDate, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
