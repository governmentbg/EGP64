...
public class TransferDeloDoc  extends IndexUIbean   {
	...
	void initData() {
		...
		try {
			...
			setSpecificsRegisterSource(Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA,Optional.of(userRegistratura)));
	...
	public void actionChangeReg() {
			...
			if(this.registratura!=null) {
				specificsRegister.put( DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA, this.registratura);
				specificsRegister.put( DocuClassifAdapter.REGISTRI_INDEX_ALG , DocuConstants.CODE_ZNACHENIE_ALG_FREE);
				//setSpecificsRegister(Collections.singletonMap(DocuClassifAdapter.REGISTRI_INDEX_REGISTRATURA,Optional.of(this.registratura)));
			}else {
