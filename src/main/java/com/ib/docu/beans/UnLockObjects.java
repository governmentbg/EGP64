...
//	private Integer periodAssign;//период на дата на поставяне
    ...
    public Integer getCodePrepiska() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO;
    }
    /** Задача " */
    public Integer getCodeZadacha() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_TASK;
    }
   
    /** Код на класификация "Тип документ" */
    public Integer getCodeDocument() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC;
    }

    /** Код на класификация "Административна структура" */
    public Integer getCodeAdmStructura() {
        return DocuConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT;
    }
    
    /** Пощенска кутия" */
    public Integer getCodeMail() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_MAILBOX;
    }
    
    /** Код на класификация "Изпълнение на процедура" */
    public Integer getCodeProcExe() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_PROC_EXE;
    }

    
    /** Код на класификация "СЕОС/ССЕВ съобщение" */
    public Integer getCodeEgovMsg() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_EGOVMESSAGE;
    }

    //Нови 26092022
    /** Спортно обединение */
    public Integer getCodeSpObed() {
    ...
    /** Спортни обекти */
    public Integer getCodeSpObj() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_SPORTS_OBJECTS;
    }
    /** Треньорски кадри */
    public Integer getCodeCoache() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_COACHES;
    }
    /** Код на класификация "Заявление Вписване" */
    public Integer getCodeVpis() {
        return DocuConstants.CODE_ZNACHENIE_JOURNAL_EGOVMESSAGE;
    }
...
//		obectList.add(new SelectItem(getCodeProcExe(),getMessageResourceString(LABELS, "docu.procedura")));//Komentirano po iskane na М.Алексиева за [Омбудсман 0015368]:
		//Za MMC     
		obectList=new ArrayList<>();
		obectList.add(new SelectItem(getCodeSpObed(),getMessageResourceString(LABELS, "unLockObjects.sportObedinenie")));
		obectList.add(new SelectItem(getCodeFormation(),getMessageResourceString(LABELS, "unLockObjects.sportFormirovanie")));
		obectList.add(new SelectItem(getCodeSpObj(),getMessageResourceString(LABELS, "unLockObjects.sportObject")));
		obectList.add(new SelectItem(getCodeCoache(),getMessageResourceString(LABELS, "unLockObjects.sportCoache")));
		obectList.add(new SelectItem(getCodeEgovMsg(),getMessageResourceString(LABELS, "monitoringSSEV.zaiavlenie")));
		obectList.add(new SelectItem(getCodeZadacha(),getMessageResourceString(LABELS, "docu.taskH1")));
		obectList.add(new SelectItem(getCodeAdmStructura(),getMessageResourceString(LABELS, "admStruct.admStruct")));
	...
	public void actionUnlock() {
		if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_MAILBOX) {
			try {
				...
				JPA.getUtil().begin();
					...
					query.setParameter(OBJTYPE, DocuConstants.CODE_ZNACHENIE_JOURNAL_MAILBOX);
					query.executeUpdate();
					JPA.getUtil().commit();
...
//			if(lockList.isEmpty()){
			...
			return;
		} else if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_SPORTS_OBJECTS) {
			qq="select  lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
					+ TIPOBJIDLOCKINF
					+ " ,so.name name, so.reg_nomer regNomer"
					+ " FROM LOCK_OBJECTS lo, mms_sport_obekt so "
					+ " WHERE lo.object_ID=so.ID AND OBJECT_TIP=:objType ";
			if (rnDoc!=null && !"".equals(rnDoc)) {
				qq+=" and UPPER(d.RN_DOC) like '%"+rnDoc.toUpperCase()+"%' ";
			}
		} else if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_SPORT_OBED) {
			qq="select  lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
					+ TIPOBJIDLOCKINF
					+ " ,ar.ref_name"
					+ " FROM LOCK_OBJECTS lo, mms_sport_obedinenie so,adm_referents ar "
					+ " WHERE lo.object_ID=so.ID AND OBJECT_TIP=:objType and so.id_Object=ar.code";
			if (rnDoc!=null && !"".equals(rnDoc)) {
				qq+=" and UPPER(d.RN_DOC) like '%"+rnDoc.toUpperCase()+"%' ";
			}
		} else if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_SPORTS_FORMATIONS) {
			qq="select  lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
					+ TIPOBJIDLOCKINF
					+ " ,ar.ref_name"
					+ " FROM LOCK_OBJECTS lo, mms_sport_formirovanie sf,adm_referents ar "
					+ " WHERE lo.object_ID=sf.ID AND OBJECT_TIP=:objType and sf.id_Object=ar.code";
			if (rnDoc!=null && !"".equals(rnDoc)) {
				qq+=" and UPPER(d.RN_DOC) like '%"+rnDoc.toUpperCase()+"%' ";
			}
		} else if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_COACHES) {
			qq="select  lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
					+ TIPOBJIDLOCKINF
					+ " ,ar.ref_name"
					+ " FROM LOCK_OBJECTS lo, mms_coaches sf,adm_referents ar "
					+ " WHERE lo.object_id =  sf.id_object  AND OBJECT_TIP=:objType and sf.id_Object=ar.code";//За разлика от горните тук връзката е lo.object_ID=sf.id_object т.е. по id_object
			if (rnDoc!=null && !"".equals(rnDoc)) {
				qq+=" and UPPER(d.RN_DOC) like '%"+rnDoc.toUpperCase()+"%' ";
			}
		} else if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_EGOVMESSAGE) {
			qq="select  lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
					+ TIPOBJIDLOCKINF
					+ " ,ar.ref_name"
					+ " FROM LOCK_OBJECTS lo, mms_vpisvane sf,adm_referents ar "
					+ " WHERE lo.object_ID=sf.ID AND OBJECT_TIP=:objType and sf.id_Object=ar.code";
			if (rnDoc!=null && !"".equals(rnDoc)) {
				qq+=" and UPPER(d.RN_DOC) like '%"+rnDoc.toUpperCase()+"%' ";
			}
		...
		} else if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_TASK) {
		...
		} else if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC) {
			qq="select  "+DocDAO.formRnDocSelect("d.", JPA.getUtil().getDbVendorName())+" rnDoc,d.DOC_DATE dataDok,TO_CHAR(d.OTNOSNO)    otnosno , lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
		...
		} else if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO) {
			qq="select  d.RN_DELO rnDelo,d.DELO_DATE  dataDel,TO_CHAR(d.DELO_NAME) naimen , lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
		...
		} else 	if (refType==DocuConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT) {
			qq="select lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
					+ TIPOBJIDLOCKINF
					+ FROMLOCKOBJLO
					+ " WHERE  OBJECT_TIP=:objType ";
		...
		} else	if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_PROC_EXE) {
			qq="select  lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
		...
		for (int i = 0; i < lockList.size(); i++) {
			...
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_SPORTS_OBJECTS) {
				...
				tmp.setSenderName((String) op[5]);

			}
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_SPORT_OBED) {
				if (op[0] instanceof java.util.Date) {
					tmp.setLockDate((java.util.Date)op[0]);
			    }
				tmp.setObjectTip(Long.parseLong(op[2].toString()));
			...
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_SPORTS_FORMATIONS) {
				if (op[0] instanceof java.util.Date) {
					tmp.setLockDate((java.util.Date)op[0]);
			    }
				tmp.setObjectTip(Long.parseLong(op[2].toString()));
			...
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_COACHES) {
				if (op[0] instanceof java.util.Date) {
					tmp.setLockDate((java.util.Date)op[0]);
			    }
				tmp.setObjectTip(Long.parseLong(op[2].toString()));
			...
			if (refType==DocuConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT) {
				tmp.setObjectId(Long.parseLong(op[3].toString()));
				tmp.setObjectTip(Long.parseLong(op[2].toString()));
				tmp.setUserId(Long.parseLong(op[1].toString()));
				if (op[0] instanceof java.util.Date) {
			...
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_EGOVMESSAGE) {
				tmp.setObjectId(Long.parseLong(op[3].toString()));
				tmp.setObjectTip(Long.parseLong(op[2].toString()));
				tmp.setUserId(Long.parseLong(op[1].toString()));
				if (op[0] instanceof java.util.Date) {
				...
				tmp.setLockInfo((String) op[4]);
				tmp.setRnDoc((String) op[5]);
//				tmp.setSenderName((String) op[6]); // ewentualno da se turi tuk mms_vpisvane.type_object
			}			
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_TASK) {
			...
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC) {
				tmp.setRnDoc((String) op[0]);
				if (op[1] instanceof java.util.Date) {
					tmp.setDateVazl((java.util.Date)op[1]);
			    }
			...
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO) {
				tmp.setRnDoc((String) op[0]);
				if (op[1] instanceof java.util.Date) {
					tmp.setDateVazl((java.util.Date)op[1]);
			    }
			...
			if (refType==DocuConstants.CODE_ZNACHENIE_JOURNAL_PROC_EXE) {
