...
public class CompAccess extends UINamingContainer {
	...
	public void initAutoCompl() throws DbErrorException {
        ...
        specs.put(DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA, ((UserData)getUserData()).getRegistratura()); // текущата регистратура на логнатия
        
        List<SystemClassif> zvena = getSystemData().queryClassification(DocuConstants.CODE_CLASSIF_ADMIN_STR, null, dateClassif, getUserData().getCurrentLang(), specs);
...
//						if (getListDeloAccess().get(i).getCodeClassif().equals(DocuConstants.CODE_CLASSIF_GROUP_EMPL)) {
...
//						if (getListDocAccess().get(i).getCodeClassif().equals(DocuConstants.CODE_CLASSIF_GROUP_EMPL)) {
...
//				DeloAccess d=new DeloAccess();
////				d.setCodeClassif(DocuConstants.CODE_CLASSIF_GROUP_EMPL);
//				d.setCodeRef(Integer.valueOf(key));
//				d.setId(getMapGroupRemoved().get(key));
//				d.setFlag(DocuConstants.CODE_DEIN_IZTRIVANE);
//				da.add(d);
//			}
		...
		if (getTreeType()==1) {
			...
			if (getTreeType()==2) {
				if (getRootGroup() == null || !saveStateTree) {
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Loading tree with classifCode:{}", DocuConstants.CODE_CLASSIF_GROUP_EMPL);
					}
					try {
						...
						Map<Integer, Object> specRegistratura = Collections.singletonMap(DocuClassifAdapter.REG_GROUP_INDEX_REGISTRATURA, ((UserData) getUserData()).getRegistratura());
						//List<SystemClassif> classifList = loadClassifList(DocuConstants.CODE_CLASSIF_GROUP_EMPL, Boolean.FALSE.equals(sortByName), null);
						List<SystemClassif> classifList = getSystemData().queryClassification(DocuConstants.CODE_CLASSIF_GROUP_EMPL, null, getDateClassif(), getLang(), specRegistratura);
			...
			if (selV == 0) { // само избраното - by default
				...
				if (selectedItem.getCodeClassif()==Constants.CODE_CLASSIF_ADMIN_STR) {
					...
					//groupSluj - trqbva da razviq...
						...
						String emplCodes = (String) getSystemData().getItemSpecific(DocuConstants.CODE_CLASSIF_GROUP_EMPL, selectedItem.getCode(), SysConstants.CODE_DEFAULT_LANG, dateClassif, DocuClassifAdapter.REG_GROUP_INDEX_MEMBERS);
						...
						for (int i = 0; i < s.length; i++) {
							...
							if (!getMapAdmSelected().containsKey(s[i])) {
	...
	public void search() throws DbErrorException {
		...
		try {
			...
			if(getTreeType() == 2) { // grupi
				codeClassif = DocuConstants.CODE_CLASSIF_GROUP_EMPL;
				// групите да са филтрирани в рамките на регистртурата
