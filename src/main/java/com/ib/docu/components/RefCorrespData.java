...
public class RefCorrespData extends UINamingContainer {
	...
	private enum PropertyKeys {
		  REF, SHOWME, EKATTE, EKATTESPEC, DOCSLIST, DOCSEARCH, DOCSELTMP, DOCSEL, SEEPERSONALDATA
	}
	...
	public void initRefCorresp() {		
		...
		if(idR != null) {
			...
			if(tmpRef != null && tmpRef.getAddress() == null) {
				...
				try {
					tmpRef.getAddress().setAddrCountry(Integer.parseInt(getSystemData().getSettingsValue("delo.countryBG")));
...
//	 	това няма да се използва, тъй като Жоро направи нов метод в DocDAO специално за кореспондентите, за да може да се изкарват всички документи към всички регистратури и да се добави колона с регистратурата на документа - 11.08.2020 г.
