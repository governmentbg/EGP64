...
public class SelectReferents extends UINamingContainer {
	...
	public void initRefComp() {	
		...
		if (role == DocuConstants.CODE_ZNACHENIE_DOC_REF_ROLE_AGREED) {
			loadOpinionList(DocuConstants.CODE_ZNACHENIE_TASK_TYPE_SAGL);
			setCodeExt(null);
		} else if (role == DocuConstants.CODE_ZNACHENIE_DOC_REF_ROLE_SIGNED) {
			loadOpinionList(DocuConstants.CODE_ZNACHENIE_TASK_TYPE_PODPIS);
			
			//Подписал в документ може да бъде само от собствена регистратура
			try {
				Integer s1 = getSystemData().getRegistraturaSetting(getUserData().getRegistratura(), DocuConstants.CODE_ZNACHENIE_REISTRATURA_SETTINGS_3);
	...
	List<SystemClassif> loadClassifList(Integer codeClassif, boolean fixPrev, String query) throws DbErrorException {
		...
		if(getCodeExt() != null ) {
			...
			specifics.put((Integer)DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA, X.of(getCodeExt())); 
