				...
				if (deloDvijUpd > 0) { // има дела които са свързан и трябва да им се оправи достъпа зададен от движението им
					...
					for (Object[] row : list) {
						...
						du.addRemoveDeloDost(deloId, true
							, DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO_DVIJ, dvijId, forDelete, forInsert, sd);
						...
						SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO_DVIJ, dvijId, ident);

						journal.setCodeAction(SysConstants.CODE_DEIN_SYS_OKA);
						journal.setDateAction(new Date());
						journal.setIdUser(getUserId());
						journal.setJoinedCodeObject1(DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO);
				...
				if (docDvijUpd > 0) { // има документи които са свързан и трябва да им се оправи достъпа зададен от движението им
					...
					for (Object[] row : list) {
						...
						du.addRemoveDocDost(docId
							, DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC_DVIJ, dvijId, forDelete, forInsert, sd);
						...
						SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC_DVIJ, dvijId, ident);
		...
		try {
			...
			if (entity.getCodeRef() != null) {
				int deleted = createNativeQuery("delete from DELO_ACCESS_ALL where OBJECT_CODE = ?1 and OBJECT_ID = ?2")
					.setParameter(1, DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO_DVIJ).setParameter(2, entity.getId()).executeUpdate();
