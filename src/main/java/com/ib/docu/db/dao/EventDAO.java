...
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_VID_EVENT;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_PARTICIPATION;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIF_ROLIA_EVENT_CHANGE;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIF_ROLIA_EVENT_IN;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIF_ROLIA_EVENT_OUT;
...
public class EventDAO extends AbstractDAO<Event> {
	...
	public SelectMetadata createSelectFilter(BaseUserData ud, List<Integer> eventCodeList, Date dateFrom, Date dateTo, String eventInfo, Integer organizator, List<Integer> codeRefList,
		...
		boolean addAccess = !ud.hasAccess(Constants.CODE_CLASSIF_BUSINESS_ROLE, DocuConstants.CODE_ZNACHENIE_BUSINESS_ROLE_EVENT_ADMIN);
		...
		if (event.getResourcesList() == null || event.getResourcesList().isEmpty()) { // на екрана няма
			if (event.getDbResourcesList() != null && !event.getDbResourcesList().isEmpty()) { // трие се ако има нещо от БД
				...
				for (Integer r : event.getDbResourcesList()) {
					if (sd.matchClassifItems(DocuConstants.CODE_CLASSIF_EVENT_RESOURCES_NOTIF, r, null)) {
			...
			if (event.getDbResourcesList() == null || event.getDbResourcesList().isEmpty()) { // но няма стари от БД
				...
				for (Integer resource : event.getResourcesList()) { // всички се явяват нови
					...
					if (sd.matchClassifItems(DocuConstants.CODE_CLASSIF_EVENT_RESOURCES_NOTIF, resource, null)) {
						notifResChanged = true;
					}
				}
	...
	private String genNewTextNotif(Event saved, SystemData systemData, Integer countryBG) throws DbErrorException {
		...
		if (saved.getResourcesList() != null && !saved.getResourcesList().isEmpty()) {
			for (Integer r : saved.getResourcesList()) {
				if (systemData.matchClassifItems(DocuConstants.CODE_CLASSIF_EVENT_RESOURCES_NOTIF, r, null)) {
					newText.append(" " + systemData.decodeItem(DocuConstants.CODE_CLASSIF_EVENT_RESOURCES, r, getUserLang(), null) + ";");
				}
			}
