...
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_PROC_DEF_STAT_DEV;
...
public class ProcDefDAO extends AbstractDAO<ProcDef> {
	...
	public ProcDef copyProcDef(Integer sourceDefId, String newName, Integer registraturaId, BaseSystemData systemData) throws DbErrorException, ObjectInUseException {
		...
		for (ProcDefEtap defEtap : defEtapList) {
			Integer sourceEtapId = defEtap.getId();
			...
			for (ProcDefTask defTask : defTaskList) {
				List<ProcDefTaskIzp> copyIzpList = new ArrayList<>();
				for (ProcDefTaskIzp izp : defTask.getTaskIzpList()) {
					copyIzpList.add(new ProcDefTaskIzp(izp, clearReferents));
				}
				...
				if (clearReferents) {
					...
					if (defTask.getAssignZveno() != null) {
			...
			// това трябва да е задължително след задачите
			createNativeQuery("insert into PROC_DEF_OPINION (DEF_ETAP_ID, TASK_TYPE, OPINION, OPTYPE)" //
				+ " select :destEtap, TASK_TYPE, OPINION, OPTYPE from PROC_DEF_OPINION where DEF_ETAP_ID = :sourceEtap") //
					.setParameter("sourceEtap", sourceEtapId).setParameter("destEtap", defEtap.getId()).executeUpdate();
		}

		// копиране и на файловете само като връзка с новата процедура
		FilesDAO filesDao = new FilesDAO(getUser());
		List<Files> files = filesDao.selectByFileObject(sourceDefId, DocuConstants.CODE_ZNACHENIE_JOURNAL_PROC_DEF);
		for (Files f : files) {
			filesDao.saveFileObject(f, copy.getId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_PROC_DEF);
		}
