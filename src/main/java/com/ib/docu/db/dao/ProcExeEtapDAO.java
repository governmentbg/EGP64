...
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_BOSS_POSITIONS;
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_PROC_BUSINESS_ROLE;
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_TASK_ONE_EMPL;
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_TASK_STATUS_ACTIVE;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_DOC_REF_ROLE_AUTHOR;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_DOC_MODE_PREV_IN;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_DOC_MODE_PREV_OUT;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_STAT_CANCEL;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_STAT_DECISION;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_STAT_EXE;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_STAT_IZP;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_STAT_IZP_SROK;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_STAT_STOP;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ETAP_STAT_WAIT;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_CONTROL_ETAP;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_DECISION_PROC;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_TASK_PROC;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIF_ROLIA_CONTR;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_PROC_BUSINESS_ROLE_AUTHOR;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_PROC_BUSINESS_ROLE_AUTHOR_BOSS;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_PROC_BUSINESS_ROLE_EXEC;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_PROC_BUSINESS_ROLE_EXEC_BOSS;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_PROC_STAT_EXE;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_TASK_STATUS_AUTO_PRIKL;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_TASK_STATUS_NEIZP;
...
import com.ib.docu.db.dto.ProcDefTaskIzp;
...
public class ProcExeEtapDAO extends AbstractDAO<ProcExeEtap> {
	 ...
	 * @param comments
	 * @param taskEndDocId
	 * @param taskCodeExec изпълнителя на задачата, която се приключва
	 * @return
	 * @throws DbErrorException
	 */
	@SuppressWarnings("unchecked")
	public ProcExeEtap completeEtap(ProcExeEtap exeEtap, SystemData sd, Boolean ok, String optional, String comments, Integer taskEndDocId, Integer taskCodeExec) throws DbErrorException {
		if (exeEtap == null //
			...
			for (ProcDefEtap defEtap : defEtapList) {
				...
				if (start) {
					...
					if (cnt.intValue() == 0) { // винаги трябва да се провери дали няма активен етап с този номер, защото тогава
						...
						exe.setLastTaskExec(taskCodeExec);
	 ...
	 */
	public Integer findCodeEmployee(Integer docId, Integer codeRef, Integer zveno, Integer emplPosition, Integer role, Integer registratura, BaseSystemData sd, boolean onlyUser, Integer taskCodeExec)
...
//			само бизнес роля
		...
		if (role != null) { // това се прави защото ролята в процедурата може да има съответсвие
			...
			if (codeExt == null) { // специфичен алгоритъм
				return findCodeEmployeeAlg(docId, taskCodeExec, role, date, sd);
			}
		...
		if (zveno != null) {
			...
			if (emplPosition != null) { // ЗВЕНО И ДЛЪЖНОСТ
				...
				for (SystemClassif item : items) {
					Integer refType = (Integer) sd.getItemSpecific(CODE_CLASSIF_ADMIN_STR, item.getCode(), CODE_DEFAULT_LANG, date, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
					...
					Integer itemPosition = (Integer) sd.getItemSpecific(CODE_CLASSIF_ADMIN_STR, item.getCode(), CODE_DEFAULT_LANG, date, DocuClassifAdapter.ADM_STRUCT_INDEX_POSITION);
		...
		if (role != null) { // САМО БИЗНЕС РОЛЯ
			...
			for (Object emplCode : emplCodes) {
				...
				Integer codeRegistratura = (Integer) sd.getItemSpecific(CODE_CLASSIF_ADMIN_STR, code, CODE_DEFAULT_LANG, date, DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA);
			...
			for (ProcDefTaskIzp izp : defTask.getTaskIzpList()) {
				try {
					defTask.getRealIzpCodes()
						.add(findCodeEmployee(exe.getDocId(), izp.getCodeRef(), izp.getZveno(), izp.getEmplPosition(), izp.getBusinessRole(), exe.getRegistraturaId(), systemData, false, exe.getLastTaskExec()));
	...
	void processTask(Task task, int exeEtapId, SystemData sd) throws DbErrorException {
		...
		if (task.getEndOpinion() != null) { // мнението на текущата задача
			...
			allStopProc &= task.getEndOpinion().equals(DocuConstants.CODE_ZNACHENIE_TASK_OPINION_STOP_PROC);

			if (task.getEndOpinion().equals(DocuConstants.CODE_ZNACHENIE_TASK_OPINION_STOP_PROC)) {
				stopProcExist = true;
			}
		}
		...
		for (Object[] row : rows) {
			...
			if (row[3] != null) { // има мнение
				...
				allStopProc &= opinion == DocuConstants.CODE_ZNACHENIE_TASK_OPINION_STOP_PROC;

				if (opinion == DocuConstants.CODE_ZNACHENIE_TASK_OPINION_STOP_PROC) {
					stopProcExist = true;
				}
			}
		}
		...
		if (!opinionMap.isEmpty() && allStopProc) { // автоматично прекратяване на процедура
			...
			if (next != null) { // !!! условният е автоматично изчислен на къде да замине.
				Integer taskCodeExec = null;
				if (task.getCodeExecs() != null && task.getCodeExecs().size() > 0) {
					taskCodeExec = task.getCodeExecs().get(0);
				}
				completeEtap(exeEtap, sd, null, next[0], next[1], task.getEndDocId(), taskCodeExec);
			...
			completeEtap(exeEtap, sd, true, null, null, task.getEndDocId(), taskCodeExec);
		}
	}
	 ...
	 * @throws DbErrorException
	 */
	private Integer findCodeEmployeeAlg(Integer docId, Integer taskCodeExec, Integer alg, Date date, BaseSystemData sd) throws InvalidParameterException, DbErrorException {
		if (alg.equals(CODE_ZNACHENIE_PROC_BUSINESS_ROLE_AUTHOR_BOSS) // Ръководител на автора на документ
			...
			for (Object row : rows) {
				...
				if (item != null) {
					Integer refType = (Integer) item.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE];
			...
			Integer authorPosition = (Integer) author.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_POSITION];
		...
		} else if (alg.equals(CODE_ZNACHENIE_PROC_BUSINESS_ROLE_EXEC_BOSS) // Ръководител на изпълнителя на задачата от предходния етап
			|| alg.equals(CODE_ZNACHENIE_PROC_BUSINESS_ROLE_EXEC)) { // или Изпълнител на задачата от предходния етап
			if (taskCodeExec == null) {
				throw new InvalidParameterException("Липсва изпълнител на задачата."); 
			}
			SystemClassif taskExec = sd.decodeItemLite(CODE_CLASSIF_ADMIN_STR, taskCodeExec, CODE_DEFAULT_LANG, date, true);
			if (taskExec == null) {
				throw new InvalidParameterException("Служител с КОД=" + taskCodeExec + " не е намерен в Административната структура.");
			}

			if (alg.equals(CODE_ZNACHENIE_PROC_BUSINESS_ROLE_EXEC)) {
				return taskExec.getCode(); // ако ни трябва само изпълнителя сме готови
			}
			Integer codeParent; // в това звено ще се търси шефа
			Integer taskExecPosition = (Integer) taskExec.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_POSITION];

			if (sd.matchClassifItems(CODE_CLASSIF_BOSS_POSITIONS, taskExecPosition, date)) { // този е шеф и неговия шеф е от
																							// горното звено
				codeParent = sd.getItemParentCode(CODE_CLASSIF_ADMIN_STR, taskExec.getCodeParent(), date);
			} else {
				codeParent = taskExec.getCodeParent();
			}
			if (codeParent == null) {
				throw new InvalidParameterException("За служител с КОД=" + taskExec.getCode() + " не е намерен ръководител в Административната структура.");
			}
	...
	private String[] processTaskDecision(ProcExeEtap exeEtap, Map<Integer, Set<Integer>> opinionMap) throws DbErrorException {
		...
		if (ok == check) {
			...
			return null; // няма съвпадение и ще се решава условен етап
		}

		if (next == null) { // има съвпадение, но това е изход от процедурата
			return new String[] { null, null }; // няма следващ и няма коментар
		}
		...
		for (ProcDefTask defTask : defTaskList) {
			...
			try {
				codeAssign = findCodeEmployee(exeEtap.getDocId(), defTask.getAssignCodeRef(), defTask.getAssignZveno(), defTask.getAssignEmplPosition(), defTask.getAssignBusinessRole(),
					exe.getRegistraturaId(), systemData, true, exe.getLastTaskExec());
			...
			if (defTask.getRealIzpCodes() != null && !defTask.getRealIzpCodes().isEmpty()) { // има явни изпълнители и те трябва
				...
				for (ProcDefTaskIzp izp : defTask.getTaskIzpList()) { // тука трябва да се напряви отделна задача за всеки
					...
					Task task = createTask(exeEtap, defTask, codeAssign, exe.getWorkDaysOnly(), systemData);
					try {
						Integer code = findCodeEmployee(exeEtap.getDocId(), izp.getCodeRef(), izp.getZveno(), izp.getEmplPosition(), izp.getBusinessRole(), exe.getRegistraturaId(), systemData, false, exe.getLastTaskExec());
						task.getCodeExecs().add(code);
					} catch (InvalidParameterException e) {
			...
			for (Task task : taskList) {
				...
				if (exeEtap.getDocId() != null) {
					...
					List<Object[]> rows = createNativeQuery("select PROCESSED, RN_DOC, DOC_DATE, PORED_DELO from DOC where DOC_ID = ?1 and DOC_TYPE = ?2") //
						.setParameter(1, exeEtap.getDocId()).setParameter(2, DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN) //
						.getResultList();
					...
					if (!rows.isEmpty() && !Objects.equals(CODE_ZNACHENIE_DA, asInteger(rows.get(0)[0]))) { // става обработен
						...
						SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC, exeEtap.getDocId(),
