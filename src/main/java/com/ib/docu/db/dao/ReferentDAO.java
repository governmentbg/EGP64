...
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_ADDR_TYPE_POSTOQNEN;
...
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO;
...
//		}
	...
	public Referent findByCodeRef(Object code) throws DbErrorException {
		Referent referent;
		try {
...
/** 
 * Търсене на участник по code - взима последния добавен запис
 * Допълнително добавя и адреса ако го има
 * 
 *  */
	
	public Referent findByCodeRefLast(Object code) throws DbErrorException {
		Referent referent;
		try {
			Query query = createQuery("select r from Referent r where r.code = :code order by r.dateReg desc ") //
				.setParameter("code", code);
	 ...
	 * @throws DbErrorException
	 */
	public List<Object[]> findByNomDoc(String nomDoc) throws DbErrorException {
		nomDoc = SearchUtils.trimToNULL_Upper(nomDoc);
		if (nomDoc == null) {
			return new ArrayList<>();
		}
		try {
			List<Object[]> rows = createNativeQuery(
				"select code, ref_name, fzl_egn, fzl_lnc, nom_doc from adm_referents where upper(nom_doc) = ?1 and ref_type = ?2")
				.setParameter(1, nomDoc).setParameter(2, CODE_ZNACHENIE_REF_TYPE_FZL).getResultList();
			return rows;
		} catch (Exception e) {
			throw new DbErrorException("Грешка при търсене на лице по номер на документ за самоличност.", e);
		}
	}
	 ...
	 * @throws DbErrorException
	 */
	public String selectNomDoc(Integer code) throws DbErrorException {
		if (code == null) {
			return null;
		}
		try {
			List<Object> rows = createNativeQuery(
				"select nom_doc from adm_referents where code = ?1 and ref_type = ?2")
				.setParameter(1, code).setParameter(2, CODE_ZNACHENIE_REF_TYPE_FZL).getResultList();
			if (rows.isEmpty()) {
				return null;
			}
			return (String) rows.get(0);
		} catch (Exception e) {
			throw new DbErrorException("Грешка при търсене на номер на документ за самоличност по код на лице.", e);
		}
	}
	...
	public List<SystemClassif> findZvenoList(Date dateAction, BaseSystemData systemData) throws DbErrorException {
		...
		Map<Integer, Object> map = Collections.singletonMap(DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE, CODE_ZNACHENIE_REF_TYPE_ZVENO);
		...
		for (SystemClassif item : items) { // липсват служители и трябва да се оправи дървото за да се покаже коректно
			...
			Integer refType = (Integer) systemData.getItemSpecific(CODE_CLASSIF_ADMIN_STR, item.getCodePrev(), getUserLang(), dateAction, DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
	...
	public void mergeCorrespondents(Integer codeRef, Integer removedCodeRef, BaseSystemData systemData) throws DbErrorException, ObjectInUseException {
		...
		SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_REFERENT, codeRef, ident.toString());
	...
	public Referent moveAdmZveno(Referent referent, Integer prev, Integer parent, Date dateAction, boolean useParentRegistratura, boolean moveOnlyChildren, BaseSystemData systemData)
		...
		if (useParentRegistratura && parent != null && parent.intValue() != 0) { // само ако се иска и то не се мести на първо
																					// ниво
			Integer parentRegistratura = (Integer) systemData.getItemSpecific(CODE_CLASSIF_ADMIN_STR, parent, getUserLang(), dateAction, DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA);
	...
	public Referent save(Referent referent) throws DbErrorException {
		...
		if (!SearchUtils.isEmpty(referent.getFzlEgn())) {
			if (referent.getFzlBirthDate() == null) { // има егн, но няма дата на раждане
				try { // и се опитваме да изчислим
					referent.setFzlBirthDate(StringUtils.birthdayFromEGN(referent.getFzlEgn()));
				} catch (Exception e) { // няма много какво да направим
					LOGGER.error("Грешка при определяне на дата на раждане по ЕГН", e);
				}
			}
			if (referent.getPol() == null) { // има егн, но няма пол
				try { // и се опитваме да изчислим
					int i = Integer.parseInt(String.valueOf(referent.getFzlEgn().charAt(8)));
					referent.setPol(i % 2 == 0 ? 1 : 2);
	 ...
	 *
	 * @param userId
	 * @param settingsList за класификация {@link DocuConstants#CODE_CLASSIF_USER_SETTINGS}
	 * @param notifsList   за класификация {@link DocuConstants#CODE_CLASSIF_NOTIFF_EVENTS_ACTIVE}
	 * @param sd
	...
	public Map<Integer, Map<Integer, Boolean>> saveUserSettings(Integer userId, List<Integer> settingsList, List<Integer> notifsList, BaseSystemData sd, X<Boolean> refreshSettings, X<Boolean> refreshNotifs) throws DbErrorException {
		try {
			...
			boolean settChange = mergeAdmUserRoles(userId, settingsList, DocuConstants.CODE_CLASSIF_USER_SETTINGS //
				, new String[] { "Включена е настройката за", "Изключена е настройката за" }, sd);

			boolean notifChange = mergeAdmUserRoles(userId, notifsList, DocuConstants.CODE_CLASSIF_NOTIFF_EVENTS_ACTIVE //
		...
		result.put(DocuConstants.CODE_CLASSIF_USER_SETTINGS, settingsMap);
		...
		result.put(DocuConstants.CODE_CLASSIF_NOTIFF_EVENTS_ACTIVE, notifsMap);
	...
	/**
	 * Изтрива лицето, ако не се среща никъде из системата.
	 * 
	 * @param codeRef
	 * @return Referent, който е изтрит, иначе NULL
	 * @throws DbErrorException 
	 * @throws ObjectInUseException 
	 */
	Referent deleteIfNotUsed(Integer codeRef) throws DbErrorException, ObjectInUseException {
		if (codeRef == null) {
			return null;
		}

		StringBuilder sql = new StringBuilder();
		sql.append(" select id_object, 91 tip from mms_sport_obedinenie where id_object = :codeRef ");
		sql.append(" union all ");
		sql.append(" select id_object, 96 tip from mms_sport_formirovanie where id_object = :codeRef ");
		sql.append(" union all ");
		sql.append(" select id_object, 101 tip from mms_sport_obekt_lice where id_object = :codeRef ");
		sql.append(" union all ");
		sql.append(" select id_object, 95 tip from mms_coaches where id_object = :codeRef ");
		sql.append(" union all ");
		sql.append(" select code_ref_corresp, 51 tip from doc where code_ref_corresp = :codeRef ");
		
		try {
			@SuppressWarnings("unchecked")
			List<Object[]> rows = createNativeQuery(sql.toString()).setParameter("codeRef", codeRef).getResultList();
			if (!rows.isEmpty()) {
				return null; // няма как да го трием
			}
		} catch (Exception e) {
			throw new DbErrorException("Грешка при търсене на данни за лице!", e);
		}
		
		Referent referent = findByCode(codeRef, new Date(), true);
		delete(referent);
		return referent;
	}
	
	public Integer findEkatteByEkatteNames(String oblName, String obshtName,String nasMestoName) {
		...
		sql.append(" WHERE ");
		sql.append("    e.ime= '"+nasMestoName+"'");
		sql.append(" AND e.oblast_ime= '"+oblName+"'");
		sql.append(" AND e.obstina_ime='"+obshtName+"'");
		
		List<Object[]> rows = createNativeQuery(sql.toString()).getResultList();
		if (rows.isEmpty()) {
			return null;
		}else {
			return SearchUtils.asInteger(rows.get(0));
		}
		
	}
	
public String findEkatteOblName(Integer kodABC) {

		if (kodABC == null)  return null;
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT ");
		sql.append("    e.ime ");
		sql.append(" FROM ");
		sql.append("    ekatte_oblasti e ");
		sql.append(" WHERE ");
		sql.append("    e.abc =  "+ kodABC);
		...
		if (rows.isEmpty()) {
			...
			return SearchUtils.asString(rows.get(0));
		}
