...
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_REISTRATURA_SETTINGS;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_JOURNAL_REGISTER;
...
//		REGISTRATURA_REGISTER
			...
			// (DocuConstants.CODE_CLASSIF_REGISTRATURI_OBJACCESS
			// ,DocuConstants.CODE_CLASSIF_REGISTRATURI_REQDOC
...
//		DOC_VID_SETTINGS
	...
	/**
	 * Намира настройките на регистратура. Използва класификация {@link DocuConstants#CODE_CLASSIF_REISTRATURA_SETTINGS}.
	 *
	 * @param registratura
	 * @param sd
	 * @return
	...
	public List<SystemClassif> loadAdmStruct(Integer registratura, boolean showEmployees, BaseSystemData sd) throws DbErrorException, UnexpectedResultException {
		...
		while (i < items.size()) { // ще се махам служители и трябва да се оправи дървото за да се покаже коректно
			...
			Object refType = specifics[DocuClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE];
			Object refRegistratura = specifics[DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA];
	...
	public void transferDeloList(List<Object[]> selected, Integer destRegistraturaId, Integer destRegisterId, BaseSystemData systemData) throws DbErrorException, ObjectInUseException {
		...
		for (Object[] row : selected) {
			...
			if (Objects.equals(delo.getDeloType(), DocuConstants.CODE_ZNACHENIE_DELO_TYPE_NOM)) {
				...
				if (indexes.isEmpty()) { // прави се нов
					...
					index.setPeriodType(DocuConstants.CODE_ZNACHENIE_SHEMA_PERIOD_DEFINED);
	...
	public void transferDocList(List<Object[]> selected, Integer destRegistraturaId, Integer destRegisterId, BaseSystemData systemData) throws DbErrorException {
		...
		for (Object[] row : selected) {
			...
			if (Objects.equals(doc.getDocType(), DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK) //
