...
public class RegistraturaGroupDAO extends AbstractDAO<RegistraturaGroup> {
	...
	static class RegistraturaReferentDAO extends AbstractDAO<RegistraturaReferent> {
		...
		void delete(RegistraturaReferent entity, Integer groupType) throws DbErrorException, ObjectInUseException {
			if (Objects.equals(groupType, DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_EMPL)) {
				entity.setCodeRefClassif(Constants.CODE_CLASSIF_ADMIN_STR);

			} else if (Objects.equals(groupType, DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS)) {
				entity.setCodeRefClassif(DocuConstants.CODE_CLASSIF_EGOV_ORGANISATIONS);
	...
	public SelectMetadata findReferentsByIdGroup(Integer groupId, Integer groupType, BaseUserData ud) {
		...
		if (Objects.equals(groupType, DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS)) {
			...
			if (Objects.equals(groupType, DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_CORRESP)) {
				select.append(" select rr.ID a0, rr.CODE_REF a1, r.REF_NAME a2, r.REF_TYPE a3, r.NFL_EIK a4 ");
				if (ud.hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_SEE_PERSONAL_DATA)) {
				...
				params.put("addrTypeCorresp", DocuConstants.CODE_ZNACHENIE_ADDR_TYPE_CORRESP);
	...
	public void saveRegistraturaReferent(RegistraturaGroup group, List<Integer> codeRefList, BaseSystemData sd) throws DbErrorException, ObjectInUseException {
		...
		for (Integer codeRef : saveList) {
			...
			if (Objects.equals(group.getGroupType(), DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_EMPL)) {
				rr.setCodeRefClassif(Constants.CODE_CLASSIF_ADMIN_STR);

			} else if (Objects.equals(group.getGroupType(), DocuConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_SEOS)) {
				rr.setCodeRefClassif(DocuConstants.CODE_CLASSIF_EGOV_ORGANISATIONS);
