...
public class ProcDefTaskIzp implements PersistentEntity {
	...
	@JournalAttr(label = "izpRole", defaultText = "Изпълнител - Роля", classifID = "" + DocuConstants.CODE_CLASSIF_TASK_REF_ROLE)
	...
	@JournalAttr(label = "businessRole", defaultText = "Изпълнител - Бизнес роля", classifID = "" + DocuConstants.CODE_CLASSIF_PROC_BUSINESS_ROLE)
	...
	/**
	 * @param izp
	 * @param clearReferents
	 */
	public ProcDefTaskIzp(ProcDefTaskIzp izp, boolean clearReferents) {
		this.izpRole = izp.izpRole;
		
		if (clearReferents) {
			if (izp.getZveno() == null) {
