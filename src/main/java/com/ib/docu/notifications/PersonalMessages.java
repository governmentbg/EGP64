...
public class PersonalMessages extends IndexUIbean implements Serializable {
	...
	void initData() {
		...
		try {
			...
			setBrNotif(dao.countUserNotifications(getUserData(UserData.class).getUserAccess(), DocuConstants.CODE_NOTIF_STATUS_NEPROCHETENA, getUserData(UserData.class).getRegistratura()));
	...
	public void actionLoadMessages() {
		...
		try {
			...
			ArrayList<UserNotifications> messages = dao.findUserNotifications(getUserData(UserData.class).getUserAccess(),DocuConstants.CODE_NOTIF_STATUS_NEPROCHETENA, getUserData(UserData.class).getRegistratura());
	...
	public String actionGoto(UserNotifications uNotif) {
		...
		if(uNotif.getCodeObject() == DocuConstants.CODE_ZNACHENIE_JOURNAL_PROC_EXE) {
