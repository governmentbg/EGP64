package com.ib.docu.notifications;
	...
	public void processNotificationAsync(@ObservesAsync Notification nb) {
		...
		try {
			...
			if (nb.getNotificationCode() != null ) {
				...
				if (!nb.getSd().matchClassifItems(DocuConstants.CODE_CLASSIF_NOTIFF_EVENTS_ACTIVE, nb.getNotificationCode(), new Date())) {
					return;
				}
				...
				for (Integer referent : nb.getAdresati()) {
					...
					if (! nb.getSd().matchClassifItems(DocuConstants.CODE_CLASSIF_USERS, referent, new Date())) {
						//референтът не е потребител - ако е за запознаване и е до структура - разбиваме
						mailList.add(referent);
						if (nb.getNotificationCode() != null && nb.getNotificationCode().equals(DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_DOC_ACCESS) ) {
							
							List<SystemClassif> children = nb.getSd().getChildrenOnNextLevel(DocuConstants.CODE_CLASSIF_ADMIN_STR , referent, new Date(), DocuConstants.CODE_DEFAULT_LANG);
							if (children != null) {
...
//			 kgMainMessages.send("тест2","-1");
	...
	public void saveMainMessage(UserNotifications msg, Set<Integer> users, Integer registraturaId, SystemData sd) {
		...
		if (msg.getMessageType() != null) {
				...
				if (msg.getMessageType().intValue() == - DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_DOC_ACCESS
					|| msg.getMessageType().intValue() == - DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_CONTROL_PROC
					|| msg.getMessageType().intValue() == - DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_CONTROL_ETAP
					|| msg.getMessageType().intValue() == - DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_DECISION_PROC
					|| msg.getMessageType().intValue() == - DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_TASK_PROC
					|| msg.getMessageType().intValue() == - DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_PROC_STOP) {
		...
		try {
			...
			for (Integer userId : users) {
					...
					if (empl != null) {
						email = SearchUtils.asString( empl.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_CONTACT_EMAIL]);
					}
					
					SystemClassif user = sd.decodeItemLite(Constants.CODE_CLASSIF_USERS, userId, SysConstants.CODE_DEFAULT_LANG, new Date(), true);
					if (user != null) {
						Integer dubl = SearchUtils.asInteger( user.getSpecifics()[DocuClassifAdapter.USERS_INDEX_DUBL_MAIL]);
						if (dubl != null && email != null && dubl.equals(DocuConstants.CODE_ZNACHENIE_DA)) {
							curMsg.setEmailTo(email);
							curMsg.setSentToEmail(DocuConstants.CODE_ZNACHENIE_STATUS_MAIL_NOT_SEND);
						}
					}
	...
	public void saveEmailMessage(UserNotifications msg, Set<Integer> slujit, SystemData sd) {
		...
		try {
			...
			for (Integer slujitId : slujit) {
					...
					String email = SearchUtils.asString( empl.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_CONTACT_EMAIL]);
					if (email == null) {
						continue;
					}
