...
public class DvijObserver {
	...
	private String[] generateOsnMsgDoc(Doc tmpDoc, SystemData sd) throws DbErrorException {
		...
		LOGGER.debug("Message for majl:"+strMsgDoc);
	...
	private void sendEmail(DocDvij dvij, SystemData systemData) throws DbErrorException {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("sendEmail:DocDvig="+dvij.toString());
		}
		...
		try {	
			...
			q.setParameter("ISOFFICIAL", DocuConstants.CODE_ZNACHENIE_DA);
			
			ArrayList<Object[]> files = (ArrayList<Object[]>) q.getResultList();
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("will be sent " + files.size() + " files");
			}
			if (files.size() > 0) {
...
//	            String escaped = StringEscapeUtils.escapeHtml("");
		...
		try {
			...
			ident.append(systemData.decodeItem(DocuConstants.CODE_CLASSIF_DOC_PREDAVANE_STATUS, newStatus, SysConstants.CODE_DEFAULT_LANG, null) + "\".");
			if (statusInfo != null) {
				ident.append(" Допълнителна информация: " + statusInfo + ".");
			}
			SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC_DVIJ, dvij.getId(), ident.toString());
