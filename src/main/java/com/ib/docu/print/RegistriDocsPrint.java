...
import com.ib.docu.db.dao.RegisterDocsPrintDAO;
...
//	        		sheet.autoSizeColumn((short)cN, true);
	        ...
	        //2 ред Техен №/Идва от
         	s="";
         	if (null!=item[5] && asInteger(item[5]).intValue()==(DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN) && 
...
//	        		sheet.autoSizeColumn((short)cN, true);
		    ...
		    if (null!=item[23] && null!=item[5] && asInteger(item[5]).intValue() == DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK ) { // Рег. номер на официалния документ, който е създаден от работния
		    	if (null!=item[35]) {
		    ...
		    if (!"".equals(s.trim()) && (null!=item[5] && asInteger(item[5]).intValue()==(DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK))  ) {
		    	if (typePr==0) {	
           ...
           //4 ред -  - Кореспондент
          	s=""; 
		    if(null!=item[5] && asInteger(item[5]).intValue()!=(DocuConstants.CODE_ZNACHENIE_DOC_TYPE_IN) && null!=item[30] && !asString(item[30]).equals("")) {
		    	if (typePr==0) {
		          	strB.delete(0,strB.length());
		          	strB.append(TR);
...
//    	        		sheet.autoSizeColumn((short)cN, true);
           ...
           if (null!=item[12] && ! asString(item[12]).equals("")) {
	        	 s = asString(item[12]);
	        	 if (!"".equals(s.trim())) {
	        		s=((SystemData)getSystemData()).decodeItems(DocuConstants.CODE_CLASSIF_ADMIN_STR_REPORTS, s, lang, new Date());
	        		if (typePr==0) {
		        		strB.delete(0,strB.length());
		         		strB.append(TR);
		   ...
		   if (null!=item[0] && (null != item[40] || null != item[41] || null != item[39])) {
			  ...
			  if(null!=item[39])
				  s+=" "+getMessageResourceString(LABELS, "dvijenie.nachin")+": "+getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_DVIJ_METHOD, asInteger(item[39]), Integer.valueOf(lang), new Date());
		   ...
		   if (null!=item[0] && (null != item[42] || null != item[43] || null != item[45])) {
			  ...
			  if(null!=item[42])
				  s+=" "+getMessageResourceString(LABELS, "dvijenie.nachin")+": "+getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_DVIJ_METHOD, asInteger(item[42]), Integer.valueOf(lang), new Date());
		   ...
		   if (null!=item[26]) { // Валидност + Дата
			   ...
			   if (asInteger(item[26]).intValue()==(DocuConstants.CODE_CLASSIF_DOC_VALID_DESTRUCT)) {// Унищожен док-т
...
//			        		sheet.autoSizeColumn((short)cN, true);
						  ...
						  if(null!=ddItem[5] && asInteger(ddItem[5]).intValue() == DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK) {
						  ...
						  if(null!=ddItem[7] && asInteger(ddItem[7]).intValue() == DocuConstants.CODE_CLASSIF_DOC_VALID_DESTRUCT ) {
						  ...
						  ///
						  if (null!=ddItem[3]) {
							  if(null!=ddItem[6] && asInteger(ddItem[6]).intValue() == DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK) {
								 s += ", "+getMessageResourceString(beanMessages, WORKL)+" ";
							  }else {
								 s += ", "+getMessageResourceString(beanMessages, OFICIALL)+" "; 
							  } 
							  ...
							  if(null!=ddItem[9] && asInteger(ddItem[9]).intValue() == DocuConstants.CODE_CLASSIF_DOC_VALID_DESTRUCT ) {
								  s += " ("+getMessageResourceString(beanMessages, "prRegDoc.destr");
		    	...
		    	} else if(asInteger(item[26]).intValue()==(DocuConstants.CODE_CLASSIF_DOC_VALID_ARCH)) {
		    			...
		    			n = 0;
						   ...
						   for (Object[]  daItem: this.archDocsList) {
							   ...
							   if(null!=daItem[0]) {
								  ...
								  if(null!=daItem[1] && asInteger(daItem[1]).intValue() == DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK) {
									  s+=", "+getMessageResourceString(beanMessages, WORKL)+" ";
								  }else {
									  s+=", "+getMessageResourceString(beanMessages, OFICIALL)+" "; 
								  }  
								  ...
								  ///
									  if (null!=daItem[9]) {
										  if(asInteger(daItem[9]).intValue() == DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK) {
											 s+=", "+getMessageResourceString(beanMessages, WORKL)+" ";
										  }else {
											 s+=", "+getMessageResourceString(beanMessages, OFICIALL)+" "; 
										  } 
			   ...
			   if (null!=this.ekzVlojeniList && !this.ekzVlojeniList.isEmpty()) {
				   ...
				   for (Object[]  vlItem: this.ekzVlojeniList) {
					   ...
					   if(null!=vlItem[2]) {
						  ...
						  s=" - ";
						  if(null!=vlItem[10])
							  s+=getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_DELO_TYPE, asInteger(vlItem[10]), lang, new Date());
	...
	//				        	sheet.autoSizeColumn((short)cN, true);
				  ...
				  if (null!=dvItem[2]) {// Дата предаване
					  ...
					  if(null!=dvItem[3])// nachin predavane
						  s+=", "+getMessageResourceString(beanMessages, "prRegDoc.mothodPred")+": "+getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_DVIJ_METHOD, asInteger(dvItem[3]), Integer.valueOf(lang), new Date()); 
				  ...
				  if (null!=dvItem[11]) {// Връщане дата
					  ...
					  if(null!=dvItem[12])// nachin връщане
						  s+=", "+getMessageResourceString(beanMessages, "prRegDoc.retMethod")+": "+getSystemData().decodeItem(DocuConstants.CODE_CLASSIF_DVIJ_METHOD, asInteger(dvItem[12]), Integer.valueOf(lang), new Date());
