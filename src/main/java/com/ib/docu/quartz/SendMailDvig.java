...
public class SendMailDvig implements Job {
	...
	public String[] generateOsnMsgDoc(Doc tmpDoc, SystemData sd) throws DbErrorException {
		...
		strMsgDoc= "<table border=\"0\">"+
			...
			"       <td  style=\"width: 110px; color: navy\"><strong>"+
					sd.decodeItem(DocuConstants.CODE_CLASSIF_DOC_TYPE, tmpDoc.getDocType(), DocuConstants.CODE_DEFAULT_LANG, tmpDoc.getDocDate()) 	+					
			"       </strong></td>"+
		    "        <td  align=left>"+
		    "            Номер/дата документ: " +
		    "		 </td>"+
			...
			"		</td>"+
			"       <td  style=\"color: navy\">"+
			sd.decodeItem(DocuConstants.CODE_CLASSIF_DOC_VID, tmpDoc.getDocVid(), DocuConstants.CODE_DEFAULT_LANG, tmpDoc.getDocDate()) 	+
			"		</td>"+
			"   </tr>";
		if (corespIme != null) {
			strMsgDoc += "   <tr>"+
		...
		if (tmpDoc.getReceivedBy() != null && tmpDoc.getReceiveMethod() != null && tmpDoc.getReceiveMethod().equals(DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL)) {
			strMsgDoc += "   <tr>"+
			"       <td align=\"left\" style=\"width: 150px\">"+
			"                Идва от:" +
			"		</td>"+
		...
		if (tmpDoc.getReferentsAuthor() != null && tmpDoc.getReferentsAuthor().size() > 0) {
			...
			for (DocReferent ref : tmpDoc.getReferentsAuthor()) {
				avtors += sd.decodeItem(DocuConstants.CODE_CLASSIF_ADMIN_STR, ref.getCodeRef(), DocuConstants.CODE_DEFAULT_LANG, tmpDoc.getDocDate()) + "; ";
			}
			strMsgDoc += "   <tr>"+
			"       <td align=\"left\" style=\"width: 150px\">"+
			"                Автор: " +
	...
	public void processCEOC(SystemData sd) throws JobExecutionException  {		
		try {
			...
			q.setParameter("DM", DocuConstants.CODE_ZNACHENIE_PREDAVANE_SEOS);
			q.setParameter("STAT", DocuConstants.DS_WAIT_PROCESSING);
			...
			for (Object[] row : rows) {		
				...
				Integer newStatus= DocuConstants.DS_WAIT_SENDING;
				...
				try {	
					...
					if (doc == null || !docId.equals(doc.getId())) {
						...
						q.setParameter("CO", DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
					...
					if (doc.getCodeRefCorresp() != null) {
						...
						if (coresp.getAddress() != null){							
							...
							if (coresp.getAddress().getEkatte() != null){
								...
								String item = sd.decodeItem(DocuConstants.CODE_CLASSIF_EKATTE, coresp.getAddress().getEkatte(), DocuConstants.CODE_LANG_BG, doc.getDocDate());
								if (item != null) {
					...
					newStatus= DocuConstants.DS_RETURNED_ERROR;
				...
				try {	
					...
					ident.append(sd.decodeItem(DocuConstants.CODE_CLASSIF_DOC_PREDAVANE_STATUS, newStatus, SysConstants.CODE_DEFAULT_LANG, null) + "\".");
					if (statusInfo != null) {
						ident.append(" Допълнителна информация: " + statusInfo + ".");
					}
					SystemJournal journal = new SystemJournal(DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC_DVIJ, dvijId, ident.toString());
	...
	public void processEmail(SystemData sd) throws JobExecutionException  {		
		try {
			...
			q.setParameter("DM", DocuConstants.CODE_ZNACHENIE_PREDAVANE_EMAIL);
			q.setParameter("STAT", DocuConstants.DS_WAIT_SENDING);
			ArrayList<Object[]> rows = (ArrayList<Object[]>) q.getResultList();
			...
			for (Object[] row : rows) {		
				...
				Integer newStatus= DocuConstants.DS_SENT;
				String statusInfo = null;
				Integer docId = SearchUtils.asInteger(row[1]);	
				Integer dvijId = SearchUtils.asInteger(row[0]);	
	...
	public EgovMessages convertDoc2eGovMessage(Doc doc, SystemData sd) throws InvalidParameterException, DbErrorException{
			...
			if (doc.getDocVid() != null) {
				String vidDoc = sd.decodeItem(DocuConstants.CODE_CLASSIF_DOC_VID, doc.getDocVid(), DocuConstants.CODE_LANG_BG, doc.getDocDate());
				if (vidDoc != null) {
