        ...
        try {
            ...
            try {
                ...
                String sqlUserNotifications = "SELECT" +
                        ...
                        " WHERE SENT_TO_EMAIL=" + DocuConstants.CODE_ZNACHENIE_STATUS_MAIL_NOT_SEND;
                ...
                for(Object[] rowMail: mailsForSending) {
                    ...
                    if (idUser == null) {
                    	...
                    	if (taskId != null) {
                    		...
                    		if (task != null) {
                    			...
                    			String vazlojitel = sd.decodeItem(DocuConstants.CODE_CLASSIF_ADMIN_STR_REPORTS, task.getCodeAssign(), DocuConstants.CODE_DEFAULT_LANG, task.getAssignDate());
                    			...
                    			if (task.getDocRequired() != null && task.getDocRequired().equals(DocuConstants.CODE_ZNACHENIE_DA)) {
                    				messageDetails += "Изисква се документ при приключване на задачата.<br>";
                    			}
                    			
                    			List<Files> files = fdao.selectByFileObject(taskId, DocuConstants.CODE_ZNACHENIE_JOURNAL_TASK);
                    			...
                    			if (task.getDocId() != null) {
	                    			files = fdao.selectByFileObject(task.getDocId(), DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
	                    			for (Files curFile : files) {
	                    				curFile = fdao.findById(curFile.getId());
                    	...
                    	if (objectCode != null && objectId != null && objectCode.equals( DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC)) {
                    		List<Files> files = fdao.selectByFileObject(objectId, DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
                			for (Files curFile : files) {
                				curFile = fdao.findById(curFile.getId());
                    ...
                    long status = DocuConstants.CODE_ZNACHENIE_STATUS_MAIL_SEND;
                    try {
                        ...
                        status = DocuConstants.CODE_ZNACHENIE_STATUS_MAIL_ERROR;
                    } 
