...
import com.ib.docu.components.CompDeloSearch;
...
public class DeloSearch extends SelectMetadata implements AuditExt {
	...
	@JournalAttr(label="codeRefLeadList",defaultText = "Водещ служител",classifID = ""+DocuConstants.CODE_CLASSIF_ADMIN_STR)
	...
	private boolean	rnDeloEQ;	// ако е true се търси по пълно съвпадение по номер
	...
	@JournalAttr(label="deloTypeArr",defaultText = "Тип на делото",classifID = ""+DocuConstants.CODE_CLASSIF_DELO_TYPE)
	private Integer[]	deloTypeArr;
	
	private Integer		notInTip; // ако има нещо значи няма да връща от този тип преписките
	
	@JournalAttr(label="statusArr",defaultText = "Статус на делото",classifID = ""+DocuConstants.CODE_CLASSIF_DELO_STATUS)
	 ...
	 * [5]-INIT_DOC_ID<br>
	 * [6]-REGISTER_ID<br>
	 * [7]-DELO_INFO<br> TODO това май не се използва по екраните и би могло да не се и връща !!!
	 * [8]-DELO_TYPE<br>
	 * [9]-WITH_SECTION<br>
	 * [10]-WITH_TOM<br>
	 * [11]-BR_TOM<br>
	...
	public void buildQueryComp(BaseUserData userData) {
		...
		select.append(" , d.WITH_SECTION a9, d.WITH_TOM a10, d.BR_TOM a11 ");
	...
	private void buildQueryDeloListInner(BaseUserData userData, int mode, boolean isFullText) {
		...
		sqlSelect.append(", z.USER_ID a7, z.LOCK_DATE a8, d.WITH_TOM a9, d.BR_TOM a10 ");
		...
		if (this.deloTypeArr != null && this.deloTypeArr.length != 0) {
			...
			sqlWhere.append((sqlWhere.length() == 0 ? WHERE : AND) + " d.DELO_TYPE != :deloTypeNom ");
			sqlParams.put("deloTypeNom", DocuConstants.CODE_ZNACHENIE_DELO_TYPE_NOM);
		}
		...
		if (this.statusArr != null && this.statusArr.length != 0) {
			...
			if (!userData.hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_UA)) {
				...
				sqlParams.put("statusList", Arrays.asList(DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_DA, DocuConstants.CODE_ZNACHENIE_DELO_STATUS_ARCHIVED_UA));
			}
		}
...
//			where.append((where.length() == 0 ? WHERE : AND) + " d.WITH_TOM=1 ");
		...
		sqlParams.put("zTip", DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO);
	...
	void addAccessRules(StringBuilder where, StringBuilder from, Map<String, Object> params, UserData userData, boolean viewMode) {
		...
		from.append(" inner join DELO_ACCESS_ALL on DELO_ACCESS_ALL.DELO_ID = d.DELO_ID ");
	...
	public SystemJournal toSystemJournal() throws DbErrorException {
		...
		dj.setCodeObject(DocuConstants.CODE_ZNACHENIE_JOURNAL_DELO);
