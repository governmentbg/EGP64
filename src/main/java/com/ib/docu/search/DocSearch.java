...
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_REGISTRATURI_OBJACCESS;
...
import com.ib.docu.components.CompDocSearch;
...
public class DocSearch extends SelectMetadata implements AuditExt {
	...
	@JournalAttr(label="registraturaId",defaultText = "Регистратура",classifID = ""+DocuConstants.CODE_CLASSIF_REGISTRATURI)
	private Integer			registraturaId;
	
	@JournalAttr(label="registerId",defaultText = "Регистър",classifID = ""+DocuConstants.CODE_CLASSIF_REGISTRI)
	private Integer			registerId;
	
	@JournalAttr(label="registerIdList",defaultText = "Регистър",classifID = ""+DocuConstants.CODE_CLASSIF_REGISTRI)
	private List<Integer>	registerIdList;			// за множествено по регистър
	
	@JournalAttr(label="sendRegistraturaList",defaultText = "Регистратура изпращач",classifID = ""+DocuConstants.CODE_CLASSIF_REGISTRATURI)
	private List<Integer>	sendRegistraturaList; 	// търсене на документи по регистратура изпращач

	@JournalAttr(label="userReg",defaultText = "Потребител, регистрирал документа",classifID = ""+DocuConstants.CODE_CLASSIF_USERS)
	private Integer userReg;

	@JournalAttr(label="codeRefCorresp",defaultText = "Лице",classifID = ""+DocuConstants.CODE_CLASSIF_REFERENTS)
	private Integer	codeRefCorresp;	// по код от класификацията
	
	@JournalAttr(label="nameCorresp",defaultText = "Име на лице")
	...
	private boolean	rnDocEQ;	// ако е true се търси по пълно съвпадение по номер на документ
	...
	@JournalAttr(label="docTypeEditList",defaultText = "Тип на документа", classifID = ""+ DocuConstants.CODE_CLASSIF_DOC_TYPE)
	private List<Integer>	docTypeEditList; // списък с позволени за актуализация, като ще се използва той ако нищо не е избрано
	@JournalAttr(label="docTypeArr",defaultText = "Тип на документа", classifID = ""+ DocuConstants.CODE_CLASSIF_DOC_TYPE)
	private Integer[]		docTypeArr	= new Integer[3];	// за да се връзва директно за компоненатата
	
	@JournalAttr(label="docVidList",defaultText = "Вид на документа", classifID = ""+ DocuConstants.CODE_CLASSIF_DOC_VID)
	private List<Integer>	docVidList;
	
	@JournalAttr(label="docTopicList",defaultText = "Тематика", classifID = ""+ DocuConstants.CODE_CLASSIF_DOC_TOPIC)
	private List<Integer>	docTopicList;

	@JournalAttr(label="validArr",defaultText = "Валидност", classifID = ""+ DocuConstants.CODE_CLASSIF_DOC_VALID)
	private Integer[]	validArr;	// валидност
	
	@JournalAttr(label="urgentArr",defaultText = "Спешност", classifID = ""+ DocuConstants.CODE_CLASSIF_URGENT)
	private Integer[]	urgentArr;	// спешен
	
	@JournalAttr(label="statusArr",defaultText = "Статус", classifID = ""+ DocuConstants.CODE_CLASSIF_DOC_STATUS)
	private Integer[]	statusArr;	// статус

	@JournalAttr(label="competence",defaultText = "Компетентност", classifID = ""+ DocuConstants.CODE_CLASSIF_COMPETENCE)
	private Integer 	competence; // компететност
	
	@JournalAttr(label="competence",defaultText = "Компетентност", classifID = ""+ DocuConstants.CODE_CLASSIF_DOC_IRREGULAR)
	...
	@JournalAttr(label="taskReferentList",defaultText = "Документи с поставена задача за",classifID = ""+DocuConstants.CODE_CLASSIF_ADMIN_STR)
	private List<Integer>	taskReferentList;	// докмент с поставена задача на

	@JournalAttr(label="docReferent",defaultText = "Служител, работил по документа",classifID = ""+DocuConstants.CODE_CLASSIF_ADMIN_STR)
	private Integer		docReferent;		// служител
	
	@JournalAttr(label="docReferentRoles",defaultText = "Роля на служител в документ",classifID = ""+DocuConstants.CODE_CLASSIF_DOC_REF_ROLE)
	...
	@JournalAttr(label="forRegId",defaultText = "Регистратура за регистрация",classifID = ""+DocuConstants.CODE_CLASSIF_REGISTRATURI)
	...
	@JournalAttr(label="docReferentRoles",defaultText = "Роля на участник в документ",classifID = ""+DocuConstants.CODE_CLASSIF_DOC_MEMBER_ROLES)
	...
	private void buildQueryDocListInner(BaseUserData userData, boolean viewMode, boolean isFullText) {
		...
		sqlSelect.append(", z.USER_ID a10, z.LOCK_DATE a11, d.COUNT_FILES a12, other.REGISTRATURA_ID a13 ");
		...
		if (this.nullWorkOffId != null) {
			...
			sqlParams.put("docTypeWrk", DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK);
		}
		...
		if (!viewMode && Boolean.TRUE.equals(this.updWorkRegOffForbidden)) { // TODO може ли тука да се оптимизира само ако не е избрано нищо или е избран работен
			sqlWhere.append((sqlWhere.length() == 0 ? WHERE : AND) + " (d.WORK_OFF_ID is null or d.DOC_TYPE != "+DocuConstants.CODE_ZNACHENIE_DOC_TYPE_WRK+") ");
		}
		...
		sqlFrom.append(" left outer join LOCK_OBJECTS z on z.OBJECT_TIP = :zTip and z.OBJECT_ID = d.DOC_ID and z.USER_ID != :zUser ");
		sqlParams.put("zTip", DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
	...
	//	String dialect = JPA.getUtil().getDbVendorName();
		...
		select.append(", " + String.valueOf(DocuConstants.CODE_ZNACHENIE_PREDAVANE_NA_RAKA) + " A9 ");   // Предаване на ръка
	...
	public void buildQueryDocListForOpis(BaseUserData userData, List<Object[]> allDocs, boolean nastrWithEkz) {
		...
		params.put("zTip", DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
	...
	public void addAccessRules(StringBuilder where, StringBuilder from, Map<String, Object> params, UserData userData, boolean viewMode) {
		...
		if (viewMode) {			
			if (userData.hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_FULL_VIEW)
				|| userData.hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_DOC_DELO_FULL_EDIT)) {
		...
		} else if (userData.hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_DOC_DELO_FULL_EDIT)) {
			return; // режим актуализация - пълен достъп, но само в текущата регистратура, която е сетната в търсенето
		}
		
		from.append(" inner join DOC_ACCESS_ALL on DOC_ACCESS_ALL.DOC_ID = d.DOC_ID ");
	...
	public SystemJournal toSystemJournal() throws DbErrorException {
		...
		dj.setCodeObject(DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC);
