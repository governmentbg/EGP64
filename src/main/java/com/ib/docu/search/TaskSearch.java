...
public class TaskSearch extends SelectMetadata {
	...
	private boolean	rnTaskEQ;	// ако е true се търси по пълно съвпадение по номер на задача
	...
	private void buildQueryTaskListInner(BaseUserData userData, boolean viewMode, boolean isFullText) {
		...
		if (this.codeExecList != null && !this.codeExecList.isEmpty()) {
			...
			if (Boolean.TRUE.equals(this.execOtgovoren)) {
				...
				sqlParams.put("execOtgovoren", DocuConstants.CODE_ZNACHENIE_TASK_REF_ROLE_EXEC_OTG);
			}
		...
		sqlParams.put("zTip", DocuConstants.CODE_ZNACHENIE_JOURNAL_TASK);
	...
	public void buildQueryTasksInDoc() {
		...
		from.append(" from TASK t LEFT JOIN DOC d ON d.DOC_ID = t.END_DOC_ID ");
	...
	private void addAccessRules(StringBuilder where, Map<String, Object> params, UserData userData, boolean viewMode) {
		if (viewMode && (userData.hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_FULL_VIEW)
						|| userData.hasAccess(DocuConstants.CODE_CLASSIF_DEF_PRAVA, DocuConstants.CODE_ZNACHENIE_DEF_PRAVA_TASK_FULL_EDIT))) {
	...
	private void addViewZvenoListAccessRules(StringBuilder where, Map<String, Object> params, UserData userData) {
		...
		params.put("emplRefType", DocuConstants.CODE_ZNACHENIE_REF_TYPE_EMPL);
