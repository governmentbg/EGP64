...
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_USER_LOCKED;
...
import com.ib.docu.db.dao.ReferentDelegationDAO;
 ...
 */
@ApplicationScoped
public class DocuIdentityStore extends IBIdentityStore {

	private static final Logger LOGGER = LoggerFactory.getLogger(DocuIdentityStore.class);
	...
	protected Optional<BaseUserData> findUserLDAP(LDAPCredential credential) throws AuthenticationException {
		...
		try {
			...
			String defaultDomain = systemData.getSettingsValue(DocuConstants.DEFAULT_LDAP_DOMAIN);
	...
	public UserData createUserData(SystemData sd, AdmUser user) throws DbErrorException {
		...
		if (row != null) {
			...
			userData.setRegistratura((Integer) row.getSpecifics()[DocuClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA]);
		...
		if (userData.getRegistratura() != null) { // пускаме само ако текущия работи в регистратура
			...
			Map<Integer, Boolean> userMailboxes = user.getAccessValues().get(DocuConstants.CODE_CLASSIF_MAILBOXES);
			if (userMailboxes != null && !userMailboxes.isEmpty()) { // само ако има зададени

				// пощенските кутии за регистратурата
				List<SystemClassif> items = sd.queryClassification(DocuConstants.CODE_CLASSIF_MAILBOXES, null, today, userData.getCurrentLang()
					, Collections.singletonMap(DocuClassifAdapter.MAILBOXES_INDEX_REGISTRATURA, userData.getRegistratura()));
