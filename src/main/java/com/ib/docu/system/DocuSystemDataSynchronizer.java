 ...
 * @author belev
 */
public class DocuSystemDataSynchronizer extends SystemDataSynchronizer {

	private static final Logger LOGGER = LoggerFactory.getLogger(DocuSystemDataSynchronizer.class);

	/**
	 * @param systemData
	 * @param seconds
	 */
	DocuSystemDataSynchronizer(BaseSystemData systemData, Long seconds) {
...
//		ADM_REFERENTS - Constants.CODE_CLASSIF_ADMIN_STR
		...
		if (!list.isEmpty()) {
			codes.add(Constants.CODE_CLASSIF_ADMIN_STR);
			codes.add(SysConstants.CODE_CLASSIF_USERS);
			codes.add(DocuConstants.CODE_CLASSIF_ADMIN_STR_REPORTS);
		}
...
//		REGISTRATURA_GROUPS
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			...
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(DocuConstants.CODE_CLASSIF_GROUP_EMPL);
			codes.add(DocuConstants.CODE_CLASSIF_GROUP_CORRESP);
		}
...
//		REGISTRI
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			...
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(DocuConstants.CODE_CLASSIF_REGISTRI);
			codes.add(DocuConstants.CODE_CLASSIF_REGISTRI_SORTED);
			codes.add(DocuConstants.CODE_CLASSIF_REGISTRATURI); // заради списъка от регистри като специфика
		} else {
//		REGISTRATURI
			list = JPA.getUtil().getEntityManager().createNativeQuery( //
				...
				.setParameter(h2DateParam, h2Date).getResultList();
			if (!list.isEmpty()) {
				codes.add(DocuConstants.CODE_CLASSIF_REGISTRATURI);
				codes.add(DocuConstants.CODE_CLASSIF_REGISTRATURI_OBJACCESS);
				codes.add(DocuConstants.CODE_CLASSIF_REGISTRATURI_REQDOC);
			}
		}

//		DOC_VID_SETTINGS - DocuConstants.CODE_CLASSIF_DOC_VID_SETTINGS
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			"select max (a.ID) as syncme from DOC_VID_SETTINGS a having max (a.DATE_REG) > :h2Date or max (a.DATE_LAST_MOD) > :h2Date") //
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(DocuConstants.CODE_CLASSIF_DOC_VID_SETTINGS);
		}
//		DOC_VID_SETTINGS - DocuConstants.CODE_CLASSIF_DOC_VID_SETTINGS - през файловете заради шаблоните
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			...
			.setParameter("objCode", DocuConstants.CODE_ZNACHENIE_JOURNAL_DOC_VID_SETT).setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(DocuConstants.CODE_CLASSIF_DOC_VID_SETTINGS);
		}
...
//		DOC_SHEMA - DocuConstants.CODE_CLASSIF_DOC_SHEMA
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			"select max (a.SHEMA_ID) as syncme from DOC_SHEMA a having max (a.DATE_REG) > :h2Date or max (a.DATE_LAST_MOD) > :h2Date") //
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(DocuConstants.CODE_CLASSIF_DOC_SHEMA);
		}

//		REGISTRATURA_MAILBOXES - DocuConstants.CODE_CLASSIF_MAILBOXES
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			...
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(DocuConstants.CODE_CLASSIF_MAILBOXES);
		}

//		PROC_DEF - DocuConstants.CODE_CLASSIF_PROCEDURI
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			"select max (a.DEF_ID) as syncme from PROC_DEF a having max (a.DATE_REG) > :h2Date or max (a.DATE_LAST_MOD) > :h2Date") //
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(DocuConstants.CODE_CLASSIF_PROCEDURI);
			codes.add(DocuConstants.CODE_CLASSIF_DOC_VID_SETTINGS);
		}
