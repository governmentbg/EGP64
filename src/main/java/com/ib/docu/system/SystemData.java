...
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_DOC_NO_PAGE_REG;
...
import static com.ib.docu.system.DocuConstants.CODE_CLASSIF_REGISTRATURI_REQDOC;
...
import com.ib.docu.notifications.PushBean;
...
public class SystemData extends BaseSystemData {
	...
	*/
	...
	private ArchimedClient archimedClient;
	...
	public SystemData() {
		...
		try {
			this.srcVersion = getSettingsValue("mms.src.version");
		} catch (Exception e) {
			LOGGER.error("Грешка при определяне на 'mms.src.version'", e);
		}
	...
	public void calculateAdditionalRegistraturi(UserData userData, Map<Integer, Map<Integer, Boolean>> accessValues) throws DbErrorException {
		...
		if (roles != null && roles.containsKey(DocuConstants.CODE_ZNACHENIE_BUSINESS_ROLE_DELOVODITEL)) { // значи е деловодител

			userData.setDelovoditel(true);

			// трябва да се добави текущата регистратура в спистка на обслужващите го
			Map<Integer, Boolean> regs = accessValues.get(DocuConstants.CODE_CLASSIF_REGISTRATURI);
			if (regs == null) { // може и да няма нито една
				regs = new HashMap<>();
				accessValues.put(DocuConstants.CODE_CLASSIF_REGISTRATURI, regs);
			}
			regs.put(userData.getRegistratura(), Boolean.TRUE);

		} else { // няма как да е деловодител
			userData.setDelovoditel(false);
			accessValues.remove(DocuConstants.CODE_CLASSIF_REGISTRATURI);
		}
...
//		if (isClassifInSetting("system.classificationsForAccessControl", CODE_CLASSIF_REGISTRATURI_OBJACCESS)) {
			...
			if (userData.isDelovoditel()) { // това е новото за да има достъп и до тях
				accessValues.get(CODE_CLASSIF_REGISTRATURI_OBJACCESS).putAll(accessValues.get(DocuConstants.CODE_CLASSIF_REGISTRATURI));
			}
//		}
	...
	public void filterRegistersByRegistratura(Integer registratura, Map<Integer, Map<Integer, Boolean>> accessValues) throws DbErrorException {
		...
		String registers = trimToNULL((String) getItemSpecific(CODE_CLASSIF_REGISTRATURI, registratura, CODE_DEFAULT_LANG, null, DocuClassifAdapter.REGISTRATURI_INDEX_REGISTRI));
	...
	public void mergeReferentsClassif(Referent referent, boolean delete) throws DbErrorException {
		...
		if (delete) { // изтривам го и ще направя един фиктивен елемент, за да сработ изтриването
			...
			DocuClassifAdapter adapter = (DocuClassifAdapter) this.classifAdapter;
	...
	protected Object createDynamicAdapterInstance() {
		return new DocuClassifAdapter(this);
	}
	...
	public NotificationPatterns getPattern(Integer eventId, Integer rolia) throws DbErrorException {
		...
		if (map == null) {
			...
			if (pattern == null) {
				pattern = map.get(DocuConstants.CODE_ZNACHENIE_NOTIF_ROLIA_ALL_DOST);
			}
			return pattern;
		}
	}
	...
	public Properties getMailProp(Integer regId, String mailBox) throws DbErrorException {
		...
		if (map == null) {
			return mailProps.get(-1).get("DEFAULT");
		}else {
...
//	}
	...
	private void loadNotifMap() throws DbErrorException {
		...
		try {
			ArrayList<Object[]> listAll = (ArrayList<Object[]>) JPA.getUtil().getEntityManager().createNativeQuery("select USER_ID, CODE_ROLE from ADM_USER_ROLES where CODE_CLASSIF = :CC").setParameter("CC", DocuConstants.CODE_CLASSIF_NOTIFF_EVENTS_ACTIVE).getResultList();
...
//			if (wsdl.indexOf("regix-service-test") != -1) { // TODO така не се прави, но на теста не работи иначе 
...
//			}
	 ...
	 * @throws DbErrorException
	 */
	public ArchimedClient getArchimedClient() throws RestClientException, DbErrorException {
		if (this.archimedClient == null) {
			LOGGER.info("START ArchimedClient initialization ...");

			String targetUrl = getSettingsValue("archimed.targetUrl");
			if (targetUrl == null) {
				throw new RestClientException("Missing Setting archimed.targetUrl!");
			}
			this.archimedClient = ArchimedClient.create(targetUrl);
			
			LOGGER.info("END ArchimedClient initialization ... success");
		}
		return this.archimedClient;
	}
}