			...
			for (Object[] row : rows) {
				...
				if (SearchUtils.asInteger(row[8]).equals(docId)) {
					LOGGER.info("вид на връзката: " //
						+ sd.decodeItem(DocuConstants.CODE_CLASSIF_DOC_REL_TYPE, SearchUtils.asInteger(row[1]), 1, null));
				} else {
					LOGGER.info("вид на връзката: " //
						+ sd.decodeItemDopInfo(DocuConstants.CODE_CLASSIF_DOC_REL_TYPE, SearchUtils.asInteger(row[1]), 1, null));
				}
			}
