...
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_VID_DOC_ZAIAVL_VPISVANE_OSK;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_VID_DOC_ZAIAVL_VPISVANE_SPORTNO_OBEDINENIE;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_VID_SPORT_OBEDINENIE_OK;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_VID_SPORT_OBEDINENIE_SF;
...
/**
 * ММС – миграция на спортни обединения
 *
 * @author belev
 */
public class T1RegSportFederations {

	private static final Logger LOGGER = LoggerFactory.getLogger(T1RegSportFederations.class);

	/**  */
	private static final String REF_MIG_NAME = "mms_sport_obedinenie"; // mms_sport_obedinenie

	/** @param args */
	public static void main(String[] args) {
		T1RegSportFederations t = new T1RegSportFederations();
...
//		t.regix(JPA.getUtil());
	...
	private Map<String, Integer>	mejdFedMap;
	...
	private String	zapOtn;

	private String dopInfo;

	/**  */
	public T1RegSportFederations() {
		this.vidSportMap = T0Start.findDecodeMap(JPA.getUtil().getEntityManager(), CODE_CLASSIF_VIDOVE_SPORT);
		this.mejdFedMap = T0Start.findDecodeMap(JPA.getUtil().getEntityManager(), CODE_CLASSIF_MMS_MEJD_FED);
	...
	public void clear(JPA jpa) {
		...
		try {
			...
			cnt = em.createNativeQuery( // после по това число ще знам кои да изтрия
				"update doc set user_last_mod = -1000 where doc_id in (select id_doc from mms_vpisvane_doc where type_object = ?1)") //
				.setParameter(1, CODE_ZNACHENIE_JOURNAL_SPORT_OBED).executeUpdate();
			LOGGER.info("   update " + cnt + " rows from table doc.user_last_mod=-1000");
			...
			cnt = em.createNativeQuery("delete from doc_referents where doc_id in (select distinct doc_id from doc where user_last_mod = -1000)").executeUpdate();
			LOGGER.info("   deleted " + cnt + " rows from table doc_referents");

			cnt = em.createNativeQuery("delete from doc where user_last_mod = -1000").executeUpdate();
			...
			jpa.begin();
			cnt = em.createNativeQuery("delete from mms_sport_obed_mf").executeUpdate();
			jpa.commit();
			LOGGER.info("   deleted " + cnt + " rows from table mms_sport_obed_mf");

			jpa.begin();
			cnt = em.createNativeQuery("delete from mms_sport_obedinenie").executeUpdate();
			jpa.commit();
			LOGGER.info("   deleted " + cnt + " rows from table mms_sport_obedinenie");
	...
	public void transfer(JPA jpa) {
		...
		try {
			...
			Query obedinenieQuery = em.createNativeQuery("insert into mms_sport_obedinenie (" //
				...
				+ ", user_reg, date_reg, status, date_status, dop_info, vid_sport_text )" //
			...
			select.append(" select 0 id, zap, splic_nom, splic_dat, eik, reg_nom, chairman "); // 0,1,2,3,4,5,6
			select.append(" , director, reprezent, sports, email, mso, polza_tr "); // 7,8,9,10,11,12
			select.append(" , REPLACE(REPLACE(name, '„', '\"'), '”', '\"') name, pk, head, adres, obl, obst, tel, web, notes "); // 13,14,15,16,17,18,19,20,21
			select.append(" , vid, web_mso, zap_otn, zap_dat, olimp, 1 f, 0 broi_sk "); // 22,23,24,25,26,27,28
			select.append(" from reg_sport_federations ");
			select.append(" union all "); // + обединени спортни клубове
			select.append(" select CAST (id as int), replace(zap_nom_dat, substring(udost_nom_dat from 8 for 14), '') zap ");
			select.append(" , substring(udost_nom_dat from 0 for 8) splic_nom, to_date(substring(udost_nom_dat from 9 for 10), 'DD.MM.YYYY') splic_dat ");
			select.append(" , eik, reg_osk, null, null, reprezent_osk, null, email, null, null, name_osk, null, head, adres ");
			select.append(" , 0, obstina, tel, web, notes, null, null, zap_nom_dat_end, null, null, 2, broi_sk ");
			select.append(" from reg_sport_osk ");
			select.append(" order by 28, 6 ");
			...
			for (Object[] row : rows) {
				...
				try {
					int current = Integer.parseInt(this.regNom.split("-")[0]);
					if (max < current) {
						max = current;
					}
				} catch (Exception e) {
					LOGGER.error("  ! reg_nom=" + this.regNom + " ! ERROR parseInt ", e);
				}
				...
				if (referent.isEmpty()) { // запис на всичко
					...
					Integer obedinenieId = saveObedinenie(obedinenieQuery, obedinenieMfQuery, row, codeRef, em); // mms_sport_obedinenie
																													// (4.2)
					saveVidSport(vidSportQuery, obedinenieId, em); // mms_vid_sport (4.3)
...
//				.setParameter(1, max + 1).setParameter(2, "obedinenie").executeUpdate();
//			if (cnt == 0) {
//				em.createNativeQuery("insert into sid (object, next_val) values (?1, ?2)") //
//					.setParameter(1, "obedinenie").setParameter(2, max + 1).executeUpdate();
//			}
	...
	public void validate(JPA jpa) {
		...
		try {
			...
			StringBuilder select = new StringBuilder();
			select.append(" select 0 id, zap, splic_nom, splic_dat, eik, reg_nom, chairman "); // 0,1,2,3,4,5,6
			select.append(" , director, reprezent, sports, email, mso, polza_tr, 1 f "); // 7,8,9,10,11,12,13
			select.append(" from reg_sport_federations ");
			select.append(" union all "); // + обединени спортни клубове
			select.append(" select CAST (id as int), replace(zap_nom_dat, substring(udost_nom_dat from 8 for 14), '') zap ");
			select.append(" , substring(udost_nom_dat from 0 for 8) splic_nom, to_date(substring(udost_nom_dat from 9 for 10), 'DD.MM.YYYY') splic_dat ");
			select.append(" , eik, reg_osk, null, null, reprezent_osk, null, email, null, null, 2 ");
			select.append(" from reg_sport_osk ");
			select.append(" order by 14, 6 ");
			...
			for (Object[] row : rows) {
				String tablename = ((Number) row[13]).intValue() == 1 ? "reg_sport_federations" : "reg_sport_osk";

				StringBuilder errors = new StringBuilder();
				String prefix = "\n\t";

				String zap1 = SearchUtils.trimToNULL((String) row[1]); // zap
				if (zap1 == null) {
					errors.append(prefix + "NULL zap");
				}

				String splicNom1 = SearchUtils.trimToNULL((String) row[2]); // splic_nom
				if (splicNom1 == null) {
					errors.append(prefix + "NULL splic_nom");
				}

				if (row[3] == null) { // splic_dat
					errors.append(prefix + "NULL splic_dat");
				} else {
					if (((Date) row[3]).getTime() > now) {
						errors.append(prefix + "AFTER TODAY splic_dat=" + row[3]);
					}
				}
				...
				String director1 = SearchUtils.trimToNULL((String) row[7]); // director
				if (director1 != null && !T0Start.isBgCommasSpaces(director1)) {
					errors.append(prefix + "NOT_BG_COMMA director=" + row[7]);
				}

//				String reprezent1 = SearchUtils.trimToNULL((String) row[8]); // reprezent
//				if (reprezent1 != null && !T0Start.isBgCommasSpaces(reprezent1)) {
//					errors.append(prefix + "NOT_BG_COMMA reprezent=" + row[8]);
//				}

				String sports = SearchUtils.trimToNULL_Upper((String) row[9]); // sports
				if (sports != null) {
					String[] split = sports.replaceAll(" ", "").split(",");
					boolean err = false;
					for (String s : split) {
						if (!this.vidSportMap.containsKey(s)) {
							err = true;
						}
					}
					if (err) {
						errors.append(prefix + "UNKNOWN sports=" + row[9] + "; classif=" + CODE_CLASSIF_VIDOVE_SPORT);
					}
				}
				...
				String mso = SearchUtils.trimToNULL_Upper((String) row[11]); // mso
				if (mso != null) {
					String[] split = mso.replaceAll(" ", "").replaceAll(";", ",").split(",");
					boolean err = false;
					for (String s : split) {
						if (!this.mejdFedMap.containsKey(s)) {
							err = true;
						}
					}
					if (err) {
						errors.append(prefix + "UNKNOWN mso=" + row[11] + "; classif=" + CODE_CLASSIF_MMS_MEJD_FED);
					}
				}

				String polzaTr = SearchUtils.trimToNULL_Upper((String) row[12]); // polza_tr
				if (polzaTr != null && !"ДА".equals(polzaTr)) {
					errors.append(prefix + "NOT_VALID polza_tr=" + row[12]);
				}
...
// 		mms_sport_obed_mf
	...
	void setupData(Object[] row) {
		...
		this.vid = null;
		this.brChlenove = null;
		this.typeSport = null;

		if (((Number) row[27]).intValue() == 1) {
			this.vid = CODE_ZNACHENIE_VID_SPORT_OBEDINENIE_SF;
			String olimp = SearchUtils.trimToNULL_Upper((String) row[26]);

			if (olimp != null) { // може и да остане празно
				this.typeSport = "ДА".equals(olimp) ? SysConstants.CODE_ZNACHENIE_DA : SysConstants.CODE_ZNACHENIE_NE;
			}

		} else {
			this.vid = CODE_ZNACHENIE_VID_SPORT_OBEDINENIE_OK;
			this.brChlenove = SearchUtils.asInteger(row[28]);
		}
		...
		if (sports != null) {
			String[] split = sports.replaceAll(" ", "").split(",");
			for (String s : split) {
				Integer vidSport = this.vidSportMap.get(s);
				if (vidSport != null) {
					this.vidSportList.add(vidSport);
				}
			}
			if (this.vidSportList.isEmpty()) {
				this.vidSportText = (String) row[9];
			}
		}

		this.statusVpisvane = null;
		this.zapOtn = SearchUtils.trimToNULL((String) row[24]); // zap_otn - спрямо това ще определям статуса

		StringBuilder dopInfoSb = new StringBuilder();
		String notes = SearchUtils.trimToNULL((String) row[21]); // notes
		if (notes != null) {
		...
		if (this.zapOtn != null) {
			...
			dopInfoSb.append(this.zapOtn);

			this.statusVpisvane = DocuConstants.CODE_ZNACHENIE_STATUS_REG_OTNETO_VPISVANE;
//			this.statusVpisvane = DocuConstants.CODE_ZNACHENIE_STATUS_REG_VPISAN;
	...
	private Integer saveDoc(Query query, Integer codeRef, EntityManager em) {
		...
		int docVid = CODE_ZNACHENIE_VID_DOC_ZAIAVL_VPISVANE_SPORTNO_OBEDINENIE;
		if (this.vid != null && this.vid.intValue() == CODE_ZNACHENIE_VID_SPORT_OBEDINENIE_OK) {
			docVid = CODE_ZNACHENIE_VID_DOC_ZAIAVL_VPISVANE_OSK;
		}
	...
	private Integer saveObedinenie(Query query, Query queryMf, Object[] row, Integer codeRef, EntityManager em) {
		...
		query.setParameter(5, this.vid); // vid
		...
		query.setParameter(9, new TypedParameterValue(StandardBasicTypes.INTEGER, this.brChlenove)); // br_chlenove
		...
		query.setParameter(15, this.vidSportText);
		query.executeUpdate();
		...
		String[] split = mso.replaceAll(" ", "").replaceAll(";", ",").split(",");
		for (String s : split) {
			Integer mejdFedCode = this.mejdFedMap.get(s);
			if (mejdFedCode != null) {
				Integer idMf = T0Start.nextVal("seq_mms_sport_obed_mf", em);

				queryMf.setParameter(1, idMf); // id
				queryMf.setParameter(2, id); // id_sport_obed
				queryMf.setParameter(3, mejdFedCode); // mejd_fed
				queryMf.setParameter(4, new TypedParameterValue(StandardBasicTypes.TIMESTAMP, null)); // date_beg
				queryMf.setParameter(5, new TypedParameterValue(StandardBasicTypes.TIMESTAMP, null)); // date_end
				queryMf.setParameter(6, this.regNom); // reg_nomer
				queryMf.setParameter(7, this.splicDate); // date_doc
				queryMf.setParameter(8, T0Start.USER); // user_reg
				queryMf.setParameter(9, this.splicDate); // date_reg
				queryMf.executeUpdate();
			}
		}
		return id;
	}
	...
	/** */
	private void saveVidSport(Query query, Integer obedinenieId, EntityManager em) {
		if (this.vidSportList.isEmpty()) {
			return;
		}
		for (Integer vidSport : this.vidSportList) {
			Integer id = T0Start.nextVal("seq_mms_vid_sport", em);

			query.setParameter(1, id); // id
			query.setParameter(2, CODE_ZNACHENIE_JOURNAL_SPORT_OBED); // tip_object
			query.setParameter(3, obedinenieId); // id_object
			query.setParameter(4, T0Start.USER); // user_reg
			query.setParameter(5, this.splicDate); // date_reg
			query.setParameter(6, vidSport); // vid_sport
			query.executeUpdate();
		}
	}
	...
	private Integer saveVpisvane(Query query, Integer obedinenieId, EntityManager em) {
		...
		query.setParameter(16, this.zapOtn);
