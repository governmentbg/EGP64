...
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_VID_DOC_ZAIAVL_VPISVANE_TD;
import static com.ib.docu.system.DocuConstants.CODE_ZNACHENIE_VID_SPORTNO_FORMIROVANIE_TD;
...
/**
 * ММС – миграция на спортни формирования ТД
 *
 * @author belev
 */
public class T2RegSportKlTd {

	private static final Logger LOGGER = LoggerFactory.getLogger(T2RegSportKlTd.class);

	/**  */
	private static final String REF_MIG_NAME = "reg_td_nostd"; // reg_td_nostd

	/** @param args */
	public static void main(String[] args) {
		T2RegSportKlTd t = new T2RegSportKlTd();
...
//		t.regix(JPA.getUtil());
	...
	public void transfer(JPA jpa) {
		...
		try {
			...
			select.append(" select 0 kl, kl.reg_nom reg_nom_sk, kl.dat_vpis reg_dat_sk, '1' reg_nom, kl.eik, kl.dat_priem priet "); // 0,1,2,3,4,5
			select.append(" , '' chairman, kl.represent, '' mem_osk, '' polza, kl.udost_nom, kl.email "); // 6,7,8,9,10,11
			select.append(" , kl.name, kl.head, null pk, kl.adres, kl.obst, kl.obl, kl.tel, '' web, '' notes "); // 12,13,14,15,16,17,18,19,20
			select.append(" , null data_end, '' motivi, '' polza_tr, '' in_tr, '' project, '' bs, ob.id ob_id "); // 21,22,23,24,25,26,27
			select.append(" from reg_td_nostd kl");
			select.append(" left outer join mms_sport_obedinenie ob on ob.reg_nomer = '1' ");
			select.append(" order by 10 ");
...
//		Трябва преди записа на клуба да се търси, има ли го вече и ако го има, да се пропусне записа.
		...
		try {
			...
			StringBuilder select = new StringBuilder();
			select.append(" select 0 kl, kl.reg_nom reg_nom_sk, kl.dat_vpis reg_dat_sk, '1' reg_nom, kl.eik, kl.dat_priem priet "); // 0,1,2,3,4,5
			select.append(" , '' chairman, kl.represent, '' mem_osk, '' polza, kl.udost_nom, kl.email, ob.id ob_id "); // 6,7,8,9,10,11,12
			select.append(" from reg_td_nostd kl ");
			select.append(" left outer join mms_sport_obedinenie ob on ob.reg_nomer = '1' ");
			select.append(" order by 10 ");
			...
			for (Object[] row : rows) {
				...
				if (priet != null && !"44511".equalsIgnoreCase(priet)) {
...
//				}
				...
				if (email1 != null && !email1.equals("не е посочен")) {
...
//		mms_vid_sport
	...
	void setupData(Object[] row) {
		...
		this.licenz = SearchUtils.trimToNULL((String) row[10]); // udost_nom;
		this.licenzDate = this.regDatSk;

		this.dateAcceptance = null;
		String priet = SearchUtils.trimToNULL((String) row[5]); // priet
		if ("44511".equalsIgnoreCase(priet)) {
	...
	private Integer saveDoc(Query query, Query queryUpdate, Integer codeRef, EntityManager em) {
		...
		query.setParameter(10, CODE_ZNACHENIE_VID_DOC_ZAIAVL_VPISVANE_TD); // doc_vid
	...
	private Integer saveFormirovanie(Query query, Integer codeRef, EntityManager em) {
		...
		query.setParameter(5, CODE_ZNACHENIE_VID_SPORTNO_FORMIROVANIE_TD); // vid
